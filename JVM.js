  ...
  rows.forEach(function(r){
    ...
    if(isRam){
      ...
      if(noteVal.indexOf('بعد الفطار')>-1||noteVal.indexOf('After Iftar')>-1){
        ...
        newTime=NORMAL_TIMES.beforeDinner||'20:00';
      } else if(noteVal.indexOf('بعد التراويح')>-1||noteVal.indexOf('After Tarawih')>-1){
        newNote=noteVal.replace('⚡ بعد التراويح','بعد الغداء').replace('بعد التراويح','بعد الغداء').replace('After Tarawih','After Lunch');
        newTime=NORMAL_TIMES.afterLunch||'14:00';
      }
...
window._refreshPostDialogBtns=function(){
  ...
  if(window._ramadanSplitDone){
    ...
    if(!cancelBtn){
      ...
      cb.onclick=window.ezCancelRamadanSplit;
      cb.style.cssText='width:100%;height:42px;border:none;border-radius:12px;font-size:13px;font-weight:800;cursor:pointer;font-family:Cairo,sans-serif;color:#fff;background:linear-gradient(145deg,#ef4444,#dc2626);box-shadow:0 4px 14px rgba(239,68,68,0.2);transition:all 0.3s;margin:4px 0';
      cb.textContent='↩️ إلغاء تقسيم رمضان';
      body.appendChild(cb);
    }
  } else {
...
function processTable(m,t,autoDuration,enableWarnings,showPostDialog,ramadanMode){
  window._ezLastTVal=t; /* حفظ t للاستخدام في تقسيم رمضان */
...
function smartDoseRecognizer(note){
  ...
  /* FIX: "قبل الاكل" alone = 1 meal. Only count=3 when الوجبات or الثلاث/3 explicitly mentioned */
  if(/قبل\s*(الوجبات)\s*|قبل\s*(الاكل|الأكل)\s*(الثلاث|3)|قبل\s*كل\s*(وجبه|وجبة)|before\s*(all\s*)?meals|ac\s*meals/i.test(s)){res.count=3;res.isBefore=true;return res;}
  if(/بعد\s*(الوجبات)\s*|بعد\s*(الاكل|الأكل)\s*(الثلاث|3)|بعد\s*كل\s*(وجبه|وجبة)|after\s*(all\s*)?meals|pc\s*meals/i.test(s)){res.count=3;return res;}
