...
/* OpenAI helpers */
function _ezGetOpenAIKey(){try{return localStorage.getItem('ez_openai_key')||'';}catch(e){return '';}}
function _ezSetOpenAIKey(k){try{localStorage.setItem('ez_openai_key',k);}catch(e){}}
function _ezGetOpenAIModel(){try{return localStorage.getItem('ez_openai_model')||'gpt-4o-mini';}catch(e){return 'gpt-4o-mini';}}
function _ezSetOpenAIModel(m){try{localStorage.setItem('ez_openai_model',m);}catch(e){}}
function _ezGetAIProvider(){try{return localStorage.getItem('ez_ai_provider')||'gemini';}catch(e){return 'gemini';}}
function _ezSetAIProvider(p){try{localStorage.setItem('ez_ai_provider',p);}catch(e){}}

/* Prompt مصمم للصيدلة — يرجع JSON فقط */
var _GEMINI_PROMPT='You are a medical dose interpreter used in Saudi pharmacies.\nYour task: convert messy Arabic/English medication dose instructions into structured JSON.\nThe text may contain:\n* Arabic or English\n* Medical abbreviations\n* Spelling mistakes\n* Short pharmacist notes\n* Numeric dose patterns\nAlways interpret the MOST LIKELY meaning used in pharmacies.\n\nLANGUAGE NOTES\nArabic spelling may contain typos.\nExamples: صباخ = صباح مساءا = مساء حبه = حبة حب = حبة\n\nDOSE WORDS (pill amount)\nحبة / حبه / قرص / كبسولة = dose 1 حبتين / قرصين = dose 2 نصف حبة = dose 0.5\nIf dose is not mentioned → dose = 1\n\nTIMING WORDS\nmorning / صباح → 09:00 noon / ظهر → 13:00 evening / مساء → 18:00 night / ليل → 21:00 bedtime / قبل النوم / hs → 22:00 empty stomach / على الريق → 07:00 (isBefore=true)\n\nFREQUENCY WORDS\nمرة / once / daily / od → count=1 every=24 مرتين / bid → count=2 every=12 ثلاث مرات / tid → count=3 every=8 أربع مرات / qid → count=4 every=6\n\nMEDICAL ABBREVIATIONS\nod = once daily bid = twice daily tid = three times daily qid = four times daily\nq12h = every 12 hours q8h = every 8 hours q6h = every 6 hours\nhs = bedtime ac = before food pc = after food prn = when needed\n\nMEAL CONTEXT\nقبل الاكل / ac → isBefore=true بعد الاكل / pc → isBefore=false\nIf not mentioned → isBefore=false\n\nNUMERIC DOSE PATTERNS\n1x1 → once daily 1x2 → twice daily 1x3 → three times daily\n1-0-1 → morning and night 0-1-0 → noon only 1-1-1 → three times daily\n\nDEFAULT VALUES\ndose = 1 count = 1 every = 24 startTime = "09:00" isBefore = false\n\nOUTPUT RULES\nReturn ONLY valid JSON.\nDo NOT add:\n* explanations\n* comments\n* markdown\n* code blocks\nReturn raw JSON only.\n\nJSON FORMAT\n{ "count": number, "startTime": "HH:MM", "every": number, "isBefore": boolean, "dose": number, "readable_ar": "Arabic description" }\n\nEXAMPLES\nInput: حبه صباحا ومساء Output: {"count":2,"startTime":"09:00","every":12,"isBefore":false,"dose":1,"readable_ar":"حبة مرتين يومياً صباحاً ومساءً"}\nInput: بعد الاكل Output: {"count":1,"startTime":"09:00","every":24,"isBefore":false,"dose":1,"readable_ar":"حبة مرة يومياً بعد الأكل"}\nInput: قبل النوم Output: {"count":1,"startTime":"22:00","every":24,"isBefore":false,"dose":1,"readable_ar":"حبة مرة قبل النوم"}\nInput: tid pc Output: {"count":3,"startTime":"08:00","every":8,"isBefore":false,"dose":1,"readable_ar":"حبة ثلاث مرات يومياً بعد الأكل"}\nInput: على الريق Output: {"count":1,"startTime":"07:00","every":24,"isBefore":true,"dose":1,"readable_ar":"حبة مرة يومياً على الريق"}\nInput: 1-0-1 Output: {"count":2,"startTime":"09:00","every":12,"isBefore":false,"dose":1,"readable_ar":"حبة صباحاً ومساءً"}';
...
async function _ezAIBatch(notes){
  var key=_ezGetGeminiKey();
...
/* OpenAI Batch: parse multiple notes */
async function _ezOpenAIBatch(notes){
  var key=_ezGetOpenAIKey();
  if(!key||notes.length===0){console.log('🤖 OpenAI Batch: no key or empty notes');return [];}
  var model=_ezGetOpenAIModel();
  console.log('🤖 OpenAI Batch: sending '+notes.length+' notes (model: '+model+')...');
  var prompt=_GEMINI_PROMPT+'\n\nParse ALL notes below. Return ONLY a JSON ARRAY (no markdown). One object per note, same order.\nIMPORTANT: ALWAYS provide startTime and every fields. If unsure, make your best guess.\n\n';
  for(var i=0;i<notes.length;i++) prompt+=(i+1)+'. "'+notes[i]+'"\n';
  ...
  for(var _retry=0;_retry<3;_retry++){
    var _ctrl=new AbortController();var _tid=setTimeout(function(){_ctrl.abort();},15000);
    try{resp=await fetch('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+key},body:body,signal:_ctrl.signal});}
    catch(_fe){clearTimeout(_tid);if(_fe.name==='AbortError'){console.error('🤖 OpenAI Timeout');throw new Error('OpenAI timeout');}throw _fe;}
    clearTimeout(_tid);
    console.log('🤖 OpenAI Attempt '+(_retry+1)+': status='+resp.status);
    if(resp.status!==429&&resp.status!==503) break;
    if(_retry<2){await new Promise(function(r){setTimeout(r,3000);});}
  }
  if(!resp||!resp.ok){
    var errText=resp?await resp.text():'No response';
    console.error('🤖 OpenAI API Error:',errText);
    throw new Error('OpenAI API '+(resp?resp.status:'?')+': '+(errText||'').substring(0,100));
  }
  var data=await resp.json();
  var text=(data.choices&&data.choices[0]&&data.choices[0].message&&data.choices[0].message.content)||'';
  console.log('🤖 OpenAI raw:',text.substring(0,200));
  text=text.replace(/```json|```/g,'').trim();
  /* OpenAI json_object mode wraps array in {items:[...]} sometimes */
  var parsed;
  try{
    parsed=JSON.parse(text);
    if(!Array.isArray(parsed)){
      /* Try common wrapper keys */
      parsed=parsed.items||parsed.doses||parsed.results||parsed.data||[parsed];
    }
  }catch(pe){
    var objs=[];var re=/\{[^{}]*"count"\s*:\s*\d[^{}]*\}/g;var m;
    while((m=re.exec(text))!==null){try{objs.push(JSON.parse(m[0]));}catch(e3){}}
    if(objs.length>0){parsed=objs;}
    else{throw new Error('Cannot parse OpenAI response');}
  }
  if(!Array.isArray(parsed)) parsed=[parsed];
  return parsed;
}

/* Unified AI batch: routes to OpenAI or Gemini, with auto-fallback */
async function _ezAIBatch(notes){
  var provider=_ezGetAIProvider();
  if(provider==='openai'&&_ezGetOpenAIKey()){
    try{return await _ezOpenAIBatch(notes);}
    catch(e){
      console.warn('AI: OpenAI failed, trying Gemini:',e.message);
      if(_ezGetGeminiKey()) return await _ezGeminiBatch(notes);
      throw e;
    }
  }
  if(_ezGetGeminiKey()){
    try{return await _ezGeminiBatch(notes);}
    catch(e){
      console.warn('AI: Gemini failed ('+e.message+'), trying OpenAI...');
      if(_ezGetOpenAIKey()) return await _ezOpenAIBatch(notes);
      throw e;
    }
  }
  console.log('AI: No key configured');
  return [];
}

/* UI: Setup AI key — supports Gemini + OpenAI */
...
function _scanPackSizeWarnings(dialogM,dialogT){
  ...
  for(var j=0;j<allItems.length;j++){
    ...
    /* Pack 28/56 = 28-based regardless of TPD */
    if(pk===28||pk===56||pk===14||pk===42||ed===28||ed===14||ed===56) has28=true;
    /* Pack 30/60/90/100/120 = 30-based */
...
function smartDoseRecognizer(note){
  ...
  res.hasE=/\b(evening|eve|night)\b|مساء|مسا|مساءا|مساءً|مسائا|مسأ|المساء|المسا|ليل|الليل|ليلا|ليلاً/i.test(s);
...
function getTimeFromWords(w){
  ...
  /* بعد/مع الاكل المبهمة → 09:00 (بعد الفطار) */
  if(/(?:[بي]عد|مع)\s*(الاكل|الأكل|الآكل|الوجبه?|الطعام)\b|after\s*(meal|food)\b|\bpc\b|\bp\.c\b/i.test(s))return{time:'09:00'};
  /* قبل الاكل المبهمة → 08:00 (قبل الفطار) */
  if(/قبل\s*(الاكل|الأكل|الآكل|الوجبه?|الطعام)\b|before\s*(meal|food)\b|\bac\b/i.test(s))return{time:'08:00'};
  
  var rules=[{test:/مع\s*(ال)?(فطار|فطور|افطار)/,time:'09:00'},{test:/مع\s*(ال)?(غدا|غداء|غذا|غذاء|عداء)/,time:'14:00'},{test:/مع\s*(ال)?(عشا|عشاء|سحور|سحر)/,time:'21:00'},{test:/مع\s*(ال)?(اكل|أكل|وجب|طعام)/,time:'09:00'},{test:/empty|stomach|ريق|الريق|الريج|الريئ|على الريق|fasting/,time:'07:00'},{test:/قبل\s*(الاكل|الأكل|meal)|before\s*(meal|food)/,time:'08:00'},{test:/before.*bre|before.*fatur|before.*breakfast|before.*iftar|قبل.*فطر|قبل.*فطار|قبل.*فطور|قبل.*افطار/,time:'08:00'},{test:/after.*bre|after.*fatur|after.*breakfast|after.*iftar|[بي]عد.*فطر|[بي]عد.*فطار|[بي]عد.*فطور|[بي]عد.*افطار/,time:'09:00'},{test:/\b(morning|am|a\.m|mane)\b|صباح|الصباح|صبح/,time:'09:00'},{test:/\b(noon|midday)\b|ظهر|الظهر|ظهرا|ظهراً/,time:'12:00'},{test:/before.*lun|before.*lunch|قبل.*غدا|قبل.*غداء|قبل.*غذا|قبل.*غذاء|قبل.*عداء|قبل.*العداء/,time:'13:00'},{test:/after.*lun|after.*lunch|[بي]عد.*غدا|[بي]عد.*غداء|[بي]عد.*غذا|[بي]عد.*غذاء|[بي]عد.*عداء|[بي]عد.*العداء/,time:'14:00'},{test:/\b(asr|afternoon|pm|p\.m)\b|عصر|العصر|عصرا|عصراً/,time:'15:00'},{test:/maghrib|مغرب|المغرب/,time:'18:00'},{test:/before.*din|before.*sup|before.*dinner|before.*asha|before.*suhoor|before.*sahoor|قبل.*عشا|قبل.*عشو|قبل.*عشاء|قبل.*سحور|قبل.*سحر/,time:'20:00'},{test:/after.*din|after.*sup|after.*dinner|after.*asha|after.*suhoor|after.*sahoor|[بي]عد.*عشا|[بي]عد.*عشو|[بي]عد.*عشاء|بعد.*سحور|بعد.*سحر/,time:'21:00'},{test:/bed|sleep|sle|نوم|النوم|نووم|hs|h\.s|nocte/,time:'22:00'},{test:/مساء|مسا|مساءا|مساءً|مسائا|مسأ|evening|eve|night|nocte|ليل|الليل|ليلا|ليلاً/,time:'21:00'}];
...
function processTable(m,t,autoDuration,enableWarnings,showPostDialog,ramadanMode){
    ...
    if(ramadanMode){
      ...
      /* ── CASE D: Once daily ── */
        ...
        if(!noteMapR && (doseRec.hasB||doseRec.hasL||doseRec.hasD||doseRec.hasM||doseRec.hasN||doseRec.hasA||doseRec.hasE||doseRec.hasBed||doseRec.hasEmpty)){
          /* CRITICAL FIX: فطار in Ramadan = iftar (19:00) NOT breakfast→suhoor (04:00)
             سحور in Ramadan = suhoor (04:00) NOT dinner→iftar (19:00)
             Check explicit Ramadan meal keywords FIRST before generic hasB/hasD mapping */
  ...
  if(_geminiNotes.length>0&&_ezGetGeminiKey()){
    ...
    _ezAIBatch(_geminiNotes).then(function(results){
