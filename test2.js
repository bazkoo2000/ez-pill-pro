javascript:(function(){
  ...
  try{var lv=localStorage.getItem(VER_KEY);if(lv!==VERSION){localStorage.setItem(VER_KEY,VERSION);if(lv)setTimeout(function(){showToast('تم إضافة زر Allocate منفصل 📦','success')},1000);}}catch(e){}
    ...
    '<div class="fey-inner">'+
      ...
      '<div style="padding:20px 22px;overflow-y:auto;max-height:calc(92vh - 100px)" id="fey_body">'+
        ...
        '<div id="fey_main">'+
          ...
          '<button id="fey_direct_alloc" style="width:100%;padding:12px 20px;border:2px solid #2563eb;border-radius:14px;cursor:pointer;font-weight:800;font-size:14px;font-family:Segoe UI,sans-serif;display:flex;align-items:center;justify-content:center;gap:8px;background:white;color:#2563eb;transition:all 0.3s;margin-bottom:8px">📦 تشغيل Allocate فقط</button>'+
        '</div>'+
  ...
  var directAllocBtn=document.getElementById('fey_direct_alloc');
  ...
  // ─── Direct Allocate Button Event ───
  directAllocBtn.addEventListener('click', async function() {
