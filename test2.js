(async function () {
  ...
  // ============ INTAKE ASSOCIATIONS (active only, sorted by time) ============
  var INTAKES = [];
  ...
  var css = '\
    ...
    #fd-bulk-panel .fd-header {\
      background: #f57c00; color: #fff; padding: 12px 15px;\
    ...
    #fd-bulk-panel .fd-tabs {\
      display: flex; background: #f5f5f5; border-bottom: 2px solid #eee;\
    }\
    #fd-bulk-panel .fd-tab {\
      flex: 1; padding: 10px; text-align: center; cursor: pointer;\
      font-weight: bold; color: #666; border: none; background: none; font-size: 13px;\
    }\
    #fd-bulk-panel .fd-tab.active {\
      color: #f57c00; border-bottom: 3px solid #f57c00; background: #fff;\
    }\
    ...
    #fd-bulk-panel input[type="text"],\
    #fd-bulk-panel select {\
    ...
    #fd-bulk-panel .fd-btn-blue { background: #1976d2; }\
    #fd-bulk-panel .fd-btn-orange { background: #f57c00; }\
    #fd-bulk-panel .fd-btn-green { background: #388e3c; }\
    #fd-bulk-panel .fd-btn-red { background: #d32f2f; }\
    ...
    #fd-bulk-panel .fd-tid { color: #999; font-size: 10px; }\
    #fd-bulk-panel .fd-dates { color: #1976d2; font-size: 11px; }\
    ...
    #fd-bulk-panel .fd-progress-bar { background: #f57c00; height: 5px; border-radius: 4px; transition: width 0.3s; }\
    #fd-bulk-panel .fd-calc-box {\
      margin-top: 8px; padding: 10px; background: #fff3e0; border: 1px solid #ffcc80;\
      border-radius: 6px;\
    }\
    ...
    #fd-bulk-panel .fd-calc-result {\
      margin-top: 6px; padding: 6px; background: #fff; border-radius: 4px;\
      font-weight: bold; color: #2e7d32; text-align: center; font-size: 13px;\
      border: 1px dashed #a5d6a7; display: none;\
    }\
    #fd-bulk-panel .fd-or-divider {\
      text-align: center; color: #999; margin: 10px 0; font-size: 11px;\
      position: relative;\
    }\
    ...
    #fd-bulk-panel .fd-search-results {\
      max-height: 150px; overflow-y: auto; border: 1px solid #ddd; border-radius: 4px;\
      margin-top: 5px; display: none;\
    }\
    #fd-bulk-panel .fd-search-item {\
      padding: 8px; cursor: pointer; border-bottom: 1px solid #f0f0f0; font-size: 12px;\
    }\
    #fd-bulk-panel .fd-search-item:hover { background: #e3f2fd; }\
    #fd-bulk-panel .fd-selected-med {\
      margin-top: 5px; padding: 8px; background: #e8f5e9; border-radius: 4px;\
      font-size: 12px; color: #2e7d32; display: none;\
    }\
    #fd-bulk-panel .fd-intake-grid {\
      max-height: 200px; overflow-y: auto; border: 1px solid #ddd; border-radius: 4px;\
      margin-top: 5px;\
    }\
    #fd-bulk-panel .fd-intake-item {\
      display: flex; align-items: center; padding: 5px 8px;\
      border-bottom: 1px solid #f0f0f0; gap: 8px; font-size: 12px;\
    }\
    #fd-bulk-panel .fd-intake-item:hover { background: #f5f5f5; }\
    #fd-bulk-panel .fd-intake-item input { width: 16px; height: 16px; }\
    #fd-bulk-panel .fd-intake-time { color: #1976d2; font-weight: bold; min-width: 45px; }\
    #fd-bulk-panel .fd-intake-dose {\
      width: 50px; padding: 3px; border: 1px solid #ccc; border-radius: 3px;\
    ...
    <div class="fd-tab-content active" id="fd-tab-update">\
      ...
      <div id="fd-date-section" style="display:none;">\
        ...
        <div class="fd-calc-box">\
          ...
          <div class="fd-row">\
            <div><label style="font-size:11px;margin:0 0 2px;">QTY</label><input type="number" id="fd-qty" value="1" min="1" /></div>\
            <div><label style="font-size:11px;margin:0 0 2px;">Size</label><input type="number" id="fd-size" value="30" min="1" /></div>\
            <div><label style="font-size:11px;margin:0 0 2px;">Days</label><input type="number" id="fd-total-days" readonly style="background:#eee;font-weight:bold;" /></div>\
          </div>\
...
\
      <label>Start Date:</label>\
      <input type="date" id="fd-add-start" />\
\
      <div class="fd-calc-box">\
        <div class="fd-calc-title">Auto-Calculate End Date</div>\
        <div class="fd-row">\
          <div><label style="font-size:11px;margin:0 0 2px;">QTY</label><input type="number" id="fd-add-qty" value="1" min="1" /></div>\
          <div><label style="font-size:11px;margin:0 0 2px;">Size</label><input type="number" id="fd-add-size" value="30" min="1" /></div>\
          <div><label style="font-size:11px;margin:0 0 2px;">Days</label><input type="number" id="fd-add-total" readonly style="background:#eee;font-weight:bold;" /></div>\
        </div>\
        <div class="fd-calc-result" id="fd-add-calc-result"></div>\
      </div>\
      <div class="fd-or-divider">OR set manually</div>\
      <label>End Date:</label>\
      <input type="date" id="fd-add-end" />\
\
...
\
      <div class="fd-row" style="margin-top:8px;">\
        <div>\
          <label style="font-size:11px;margin:0 0 2px;">Every (days)</label>\
          <input type="number" id="fd-add-interval" value="1" min="1" />\
        </div>\
        <div>\
          <label style="font-size:11px;margin:0 0 2px;">Notes</label>\
          <input type="text" id="fd-add-notes" placeholder="Optional" />\
        </div>\
      </div>\
\
  ...
  // ============ TABS ============
  var tabs = panel.querySelectorAll(".fd-tab");
  ...
  function log(msg, cls) { logTo("fd-log", msg, cls); }
  function addLog(msg, cls) { logTo("fd-add-log", msg, cls); }
  function clearLog(elId) { document.getElementById(elId || "fd-log").innerHTML = ""; }
  function setProgress(pct) { document.getElementById("fd-progress-bar").style.width = pct + "%"; }
  ...
  function recalcEndDate() {
    ...
    document.getElementById("fd-total-days").value = totalDays > 0 ? totalDays : "";
    ...
    if (startDate && totalDays > 0) {
      ...
      resultEl.innerHTML = "End: " + formatDateDisplay(endDate) + " (" + totalDays + " days)";
    } else { resultEl.style.display = "none"; }
  }

  // ============ AUTO-CALC END DATE (Add tab) ============
  function recalcAddEndDate() {
    var startDate = document.getElementById("fd-add-start").value;
    var qty = parseInt(document.getElementById("fd-add-qty").value) || 0;
    var size = parseInt(document.getElementById("fd-add-size").value) || 0;
    var totalDays = qty * size;
    document.getElementById("fd-add-total").value = totalDays > 0 ? totalDays : "";
    var resultEl = document.getElementById("fd-add-calc-result");
    if (startDate && totalDays > 0) {
      var endDate = addDays(startDate, totalDays);
      document.getElementById("fd-add-end").value = endDate;
      resultEl.style.display = "block";
      resultEl.innerHTML = "End: " + formatDateDisplay(endDate) + " (" + totalDays + " days)";
    } else { resultEl.style.display = "none"; }
  }
  ...
  document.getElementById("fd-med-search").addEventListener("input", function () {
    ...
    searchTimeout = setTimeout(function () { searchMedicine(query); }, 400);
  });
  ...
  async function searchMedicine(query) {
    ...
    try {
      var resp = await apiFetch(API_ROOT + "/medicines/search?query=" + encodeURIComponent(query) + "&with_count=false", {
        headers: { Authorization: "Bearer " + token, Accept: "application/json, text/plain, */*" },
      });

      if (!resp.ok) { addLog("Search failed: " + resp.status, "err"); return; }
      ...
      resultsEl.querySelectorAll(".fd-search-item").forEach(function (item) {
        item.onclick = function () {
          ...
          document.getElementById("fd-selected-med").innerHTML =
            "Selected: <strong>" + selectedMedicine.code + "</strong> - " + selectedMedicine.name;
          document.getElementById("fd-search-results").style.display = "none";
          document.getElementById("fd-med-search").value = selectedMedicine.code + " - " + selectedMedicine.name;
          // Auto-set size from pills_per_pack if available
          if (selectedMedicine.pills_per_pack && selectedMedicine.pills_per_pack > 1) {
            document.getElementById("fd-add-size").value = selectedMedicine.pills_per_pack;
            recalcAddEndDate();
          }
        };
      });
  ...
  async function loadTreatments() {
    ...
    log("Loading...", "info");
    var token = getAuthToken();
    if (!token) { log("Token not found. Click any link in the app, then try again.", "err"); return false; }
  ...
  document.getElementById("fd-update-btn").onclick = async function () {
    ...
    clearLog("fd-log");
    log("Updating " + selected.length + "...", "info");
    ...
    for (var s = 0; s < selected.length; s++) {
      ...
      try {
        ...
        if (!getResp.ok) { log("[" + tName + "] Load fail: " + getResp.status, "err"); failed++; continue; }
        ...
        sd.setUTCDate(sd.getUTCDate() - 1); sd.setUTCHours(21, 0, 0, 0);
  ...
  document.getElementById("fd-add-btn").onclick = async function () {
    ...
    var intervalDays = parseInt(document.getElementById("fd-add-interval").value) || 1;
    var notes = document.getElementById("fd-add-notes").value || "";

    // Build configs from selected intakes
    var configs = [];
    intakeChecks.forEach(function (cb) {
      ...
      var minutesInterval = intervalDays * 24 * 60;

      // Build first_take from start date + intake time
      var ftParts = startDate.split("-");
      var firstTake = startDate + " " + String(hour).padStart(2, "0") + ":" + String(minute).padStart(2, "0");

      configs.push({
        first_take: firstTake,
        firstTake8601: "",
        dose: dose,
        days_to_take: null,
        range_to_take: null,
    ...
    if (!confirm("Add treatment?\nMedicine: " + selectedMedicine.code + " - " + selectedMedicine.name +
      "\nStart: " + startDate + "\nEnd: " + endDate +
      "\nDose times: " + intakeChecks.length + "\nEvery: " + intervalDays + " day(s)")) return;
