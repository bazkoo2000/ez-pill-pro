...
function getTimeFromWords(w){
  ...
  if(/(?:[بي]عد|مع)\s*(الاكل|الأكل|الوجبه?|الطعام)\b|after\s*(meal|food)\b|\bpc\b/i.test(s))return{time:'09:00'};
  /* قبل الاكل المبهمة → 08:00 (قبل الفطار) */
  if(/قبل\s*(الاكل|الأكل|الوجبه?|الطعام)\b|before\s*(meal|food)\b|\bac\b/i.test(s))return{time:'08:00'};
  
  var rules=[{test:/مع\s*(ال)?(فطار|فطور|افطار)/,time:'09:00'},{test:/مع\s*(ال)?(غدا|غداء|غذا|غذاء)/,time:'14:00'},{test:/مع\s*(ال)?(عشا|عشاء|سحور|سحر)/,time:'21:00'},{test:/مع\s*(ال)?(اكل|أكل|وجب|طعام)/,time:'09:00'},{test:/empty|stomach|ريق|الريق|على الريق|fasting/,time:'07:00'},{test:/قبل\s*(الاكل|الأكل|meal)|before\s*(meal|food)/,time:'08:00'},{test:/before.*bre|before.*fatur|before.*breakfast|before.*iftar|قبل.*فطر|قبل.*فطار|قبل.*فطور|قبل.*افطار/,time:'08:00'},{test:/after.*bre|after.*fatur|after.*breakfast|after.*iftar|[بي]عد.*فطر|[بي]عد.*فطار|[بي]عد.*فطور|[بي]عد.*افطار/,time:'09:00'},{test:/\b(morning|am|a\.m)\b|صباح|الصباح|صبح/,time:'09:00'},{test:/\b(noon|midday)\b|ظهر|الظهر|ظهرا|ظهراً/,time:'12:00'},{test:/before.*lun|before.*lunch|قبل.*غدا|قبل.*غداء|قبل.*غذا|قبل.*غذاء/,time:'13:00'},{test:/after.*lun|after.*lunch|[بي]عد.*غدا|[بي]عد.*غداء|[بي]عد.*غذا|[بي]عد.*غذاء/,time:'14:00'},{test:/\b(asr|afternoon|pm|p\.m)\b|عصر|العصر/,time:'15:00'},{test:/maghrib|مغرب|المغرب/,time:'18:00'},{test:/before.*din|before.*sup|before.*dinner|before.*asha|before.*suhoor|before.*sahoor|قبل.*عشا|قبل.*عشو|قبل.*عشاء|قبل.*سحور|قبل.*سحر/,time:'20:00'},{test:/after.*din|after.*sup|after.*dinner|after.*asha|after.*suhoor|after.*sahoor|[بي]عد.*عشا|[بي]عد.*عشو|[بي]عد.*عشاء|بعد.*سحور|بعد.*سحر/,time:'21:00'},{test:/bed|sleep|sle|نوم|النوم|hs|h\.s/,time:'22:00'},{test:/مساء|مسا|مساءا|مساءً|evening|eve|night|ليل|الليل|ليلا|ليلاً/,time:'21:00'}];
...
function _ezApplyUnify(group,choice,allRowsData){
  ...
  if(choice==='meal'){
    ...
    /* و+time keeping و */
    var _timeToMeal2=[
      {from:/و(الصباح|صباحا|صباحاً|صباح)/gi,to:'وبعد الفطار'},
      {from:/و(الظهر|ظهرا|ظهراً|العصر|عصرا|عصراً|عصر)/gi,to:'وبعد الغداء'},
      {from:/و(المساء|مساءا|مساءً|مساء|مسا|الليل|ليلا|ليلاً|ليل)/gi,to:'وبعد العشاء'}
    ];
    for(var _tm=0;_tm<_timeToMeal2.length;_tm++) replacements.push(_timeToMeal2[_tm]);
  } else {
    /* Step 1: replace بعد+meal */
    replacements=[
    ...
    /* Step 2: replace و+meal (keeping و) */
    var _mealToTime2=[
      {from:/و(ال)?(فطار|فطور)/gi,to:'وصباحاً'},
      {from:/و(ال)?(غدا[ءئ]?|غذا[ءئ]?)/gi,to:'وعصراً'},
      {from:/و(ال)?(عشا[ءئ]?)/gi,to:'ومساءً'}
    ];
    for(var _mt=0;_mt<_mealToTime2.length;_mt++) replacements.push(_mealToTime2[_mt]);
  }
