javascript:(function(){
  ...
  panel.innerHTML =
    '<div class="fey-inner">'+
      '<div style="background:linear-gradient(135deg,#4c1d95,#6d28d9);padding:20px 22px 18px;color:white;position:relative;overflow:hidden">'+
        ...
        '</div>'+
        '<div style="text-align:right;margin-top:4px;position:relative;z-index:1"><span style="display:inline-block;background:rgba(167,139,250,0.25);color:#c4b5fd;font-size:10px;padding:2px 8px;border-radius:6px;font-weight:700">Order Injector v3.1</span></div>'+
      '</div>'+
  ...
  // ─── إيجاد index عمود Current Flow (محسّن) ───
  function getFlowColIndex() {
    // الطريقة 1: البحث عن span.ant-table-column-title داخل thead
    var colSpans = document.querySelectorAll('thead span.ant-table-column-title');
    for (var s = 0; s < colSpans.length; s++) {
      var spanText = (colSpans[s].innerText || colSpans[s].textContent || '').replace(/\s+/g,' ').trim();
      if (spanText === 'Current Flow') {
        // نطلع لأعلى حتى نلاقي الـ th ونحسب ترتيبه
        var th = colSpans[s].closest('th');
        if (th) {
          var allTh = th.parentElement.querySelectorAll('th');
          for (var idx = 0; idx < allTh.length; idx++) {
            if (allTh[idx] === th) return idx;
          }
        }
      }
    }

    // الطريقة 2: البحث في كل th مباشرة عن النص
    var allHeaders = document.querySelectorAll('thead th');
    for (var h = 0; h < allHeaders.length; h++) {
      var headerText = (allHeaders[h].innerText || allHeaders[h].textContent || '').replace(/\s+/g,' ').trim();
      if (headerText.indexOf('Current Flow') !== -1) return h;
    }

    // الطريقة 3: البحث في كل صفوف thead (لو فيه أكثر من صف)
    var allRows = document.querySelectorAll('thead tr');
    for (var r = 0; r < allRows.length; r++) {
      var cells = allRows[r].querySelectorAll('th');
      for (var c = 0; c < cells.length; c++) {
        var cellText = (cells[c].innerText || cells[c].textContent || '').replace(/\s+/g,' ').trim();
        if (cellText.indexOf('Current Flow') !== -1) return c;
      }
    }

    // الطريقة 4: البحث عن أي عنصر يحتوي "Current Flow" داخل thead واستنتاج العمود
    var anyMatch = document.querySelector('thead *');
    if (anyMatch) {
      var allInHead = document.querySelectorAll('thead *');
      for (var a = 0; a < allInHead.length; a++) {
        var el = allInHead[a];
        // تحقق أنه عنصر leaf (ما عنده أبناء فيهم نص)
        if (el.children.length === 0 || el.tagName === 'SPAN') {
          var elText = (el.innerText || el.textContent || '').replace(/\s+/g,' ').trim();
          if (elText === 'Current Flow') {
            var parentTh = el.closest('th');
            if (parentTh) {
              var siblings = parentTh.parentElement.querySelectorAll('th');
              for (var si = 0; si < siblings.length; si++) {
                if (siblings[si] === parentTh) return si;
              }
            }
          }
        }
  ...
  async function checkRow(row) {
    ...
    if (!cb) {
      ...
      if (firstCell) { firstCell.click(); await wait(60); }
      return;
    }
  ...
  // ─── تحديد صفوف Loading Task فقط (تتجاهل Delivery وغيرها) ───
  async function selectOnlyLoadingTask() {
    var col = getFlowColIndex();
    if (col === -1) {
    ...
    for (var r = 0; r < rows.length; r++) {
      ...
      if (status.indexOf('loading task') !== -1) {
        await checkRow(rows[r]);
        count++;
        await wait(50);
      } else {
        // تأكد إن الصفوف اللي مش Loading Task مش معلّمة
  ...
  async function selectRowsByStatus(status) {
    ...
    var count = 0;
    for (var r = 0; r < rows.length; r++) {
      if (getRowStatus(rows[r], col).indexOf(status.toLowerCase()) !== -1) {
        await checkRow(rows[r]);
        count++;
        await wait(50);
      }
  ...
  async function clickRefresh() {
    ...
    if (!found) {
      ...
      for (var j = 0; j < all.length; j++) {
        ...
        if (txt === 'Refresh' && all[j].offsetParent !== null) { found = all[j]; break; }
      }
    }
    if (found) {
      found.click();
  ...
  // ─── انتظار اختفاء حالة معينة (مع auto-refresh) ───
  async function waitUntilStatusGone(status, maxTries, intervalMs) {
    ...
    for (var i = 0; i < maxTries; i++) {
      ...
      setSt('⏳ لا يزال ' + cnt + ' طلب بحالة "' + status + '" — انتظار... (' + (i+1) + '/' + maxTries + ')', 'working');
  ...
  async function runAllocate() {
    ...
    for (var r = 0; r < rows.length; r++) {
      var cellText = getRowStatus(rows[r], flowColIndex);
      if (cellText.indexOf('allocation') !== -1) {
        await checkRow(rows[r]);
        checkedCount++;
        await wait(50);
      }
  ...
  // ═══════════════════════════════════════════════════════════════
  //  🔄 Post-Allocate Flow — Loading Task فقط
  // ═══════════════════════════════════════════════════════════════
  ...
  async function runPostAllocate(didAllocate) {
    ...
    // ── المرحلة 2: تحديد طلبات Loading Task فقط ──
    setSt('☑️ تحديد طلبات Loading Task فقط...', 'working');
    var ltCount = await selectOnlyLoadingTask();
    ...
    var deliverySkipped = countRowsByStatus('delivery');
    if (deliverySkipped > 0) {
      showToast('تخطي ' + deliverySkipped + ' طلب Delivery (جاهزة)', 'info');
    }
    ...
    // ── المرحلة 4: Refresh + تحديد Loading Task فقط + Pending ──
    setSt('🔄 تحديث بعد Assign...', 'working');
    await clickRefresh();
    await wait(500);

    setSt('☑️ تحديد Loading Task فقط قبل Pending...', 'working');
    var selCount = await selectOnlyLoadingTask();
    showToast('تم تحديد ' + selCount + ' طلب Loading Task', 'success');
    await wait(500);
    ...
    // ── المرحلة 5: Refresh + تحديد Loading Task فقط + Collected ──
    setSt('🔄 تحديث بعد Pending...', 'working');
    await clickRefresh();
    await wait(500);

    setSt('☑️ تحديد Loading Task فقط قبل Collected...', 'working');
    selCount = await selectOnlyLoadingTask();
    showToast('تم تحديد ' + selCount + ' طلب Loading Task', 'success');
    await wait(500);
    ...
    var remainingLT = countRowsByStatus('loading task');

    if (deliveryCount > 0) {
      showToast('🎉 ' + deliveryCount + '/' + totalRows + ' طلب وصلوا Delivery!', 'success');
      setSt('🎉 تم بنجاح! ' + deliveryCount + ' Delivery' + (remainingLT > 0 ? ' + ' + remainingLT + ' لا يزال Loading Task' : '') + ' ✅', 'done');
    } else {
