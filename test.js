  ...
  function continueProcessing(){
    ...
    for(var i=0;i<allRowsData.length;i++){
      ...
      if(ramadanMode&&rd.dui&&rd.dui.type==='ramadan_two'){
        ...
        /* Fixed-size codes keep their fixed value even in Ramadan duplicate mode */
        if(rd.hasFixedSize){
          var _fxRm2=rd.fixedSizeBreak||fixedSizeCodes[rd.itemCode];
          ramadanRtd.push({row:r_node,info:rd.dui,calcDays:_fxRm2,isFixed:true});
        } else {
      ...
      if(ramadanMode&&rd.ramadanInfo&&(rd.ramadanInfo.type==='once_ramadan'||rd.ramadanInfo.type==='weekly_ramadan')){
        ...
        /* Single dose Ramadan: apply Ramadan time, size = rmDaysLeft OR fixedSize */
        ...
        /* Fixed-size codes keep their fixed value even in Ramadan */
        if(rd.hasFixedSize){
          var _fxRm=rd.fixedSizeBreak||fixedSizeCodes[rd.itemCode];
          setSize(tds_nodes[si_main],_fxRm);
        } else {
...
.ez-actions{display:flex;gap:7px;margin-top:4px;padding:6px 16px 10px;flex-shrink:0}\
