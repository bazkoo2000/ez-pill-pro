javascript:(function(){
  ...
  ui.innerHTML=`
    <div id="baz-header">
      <div class="hdr-left">
        ...
        <div><span class="hdr-title">البحث الشامل</span><br><span class="hdr-ver">v13 — Lightning Speed ⚡</span></div>
      </div>
      <div class="hdr-btns">
  ...
  /* التعديل الجوهري للبحث بسرعة البرق (Concurrent Fetching) */
  const runSearch=async(statusKeys)=>{
    ...
    // تجميع الطلبات لتعمل في نفس اللحظة (بالتوازي)
    const fetchPromises = statusKeys.map(async (status) => {
      ...
      try {
        ...
        if(cancelSearch) return; // التأكد بعد العودة من الطلب
