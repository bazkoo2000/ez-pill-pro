(function(){
    ...
    /* ══════════════════════════════════════════
       🔐 ENCRYPTION / DECRYPTION — مع Fallback لـ HTTP
       ══════════════════════════════════════════ */
    function _ezIsSecureContext(){
        return window.crypto && window.crypto.subtle;
    }

    /* Fallback: XOR بسيط لو مافيش crypto.subtle (HTTP) */
    function _ezXorEncode(text){
        var key=_EK;
        var out='';
        ...
        var bytes=[];
        for(var j=0;j<out.length;j++) bytes.push(out.charCodeAt(j));
        var chunks=[];
        for(var k=0;k<bytes.length;k+=8192){
            var sl=bytes.slice(k,k+8192);
            chunks.push(String.fromCharCode.apply(null,sl));
        }
        return 'XOR:'+btoa(chunks.join(''));
    }

    function _ezXorDecode(b64){
        var raw=atob(b64.replace(/^XOR:/,''));
        var key=_EK;
        var out='';
    ...
    async function _ezEncrypt(text){
        if(!_ezIsSecureContext()) return _ezXorEncode(text);
        ...
        for(var i=0;i<combined.length;i+=8192){
            var slice=combined.subarray(i,Math.min(i+8192,combined.length));
            var str='';
            for(var j=0;j<slice.length;j++) str+=String.fromCharCode(slice[j]);
            chunks.push(str);
        }
    ...
    async function _ezDecrypt(b64){
        b64=b64.trim();
        if(b64.startsWith('XOR:')) return _ezXorDecode(b64);
        if(!_ezIsSecureContext()) return _ezXorDecode(b64);
        try{
            var key=await _ezDeriveKey();
            ...
            /* لو فشل AES جرب XOR fallback */
            try{ return _ezXorDecode(b64); }catch(e2){ throw e; }
        }
    }
    ...
    /* ══════════════════════════════════════════
       👤 ADMIN PANEL — رفع + دمج ملف أدوية جديد
       ══════════════════════════════════════════ */
    function openAdminPanel(){
        ...
        '<div style="padding:18px 22px;border-bottom:1px solid rgba(129,140,248,0.08);display:flex;align-items:center;gap:10px">'+
          ...
          '<div style="flex:1"><div style="font-size:15px;font-weight:900;color:#1e1b4b">إدارة قاعدة الأدوية</div><div style="font-size:10px;font-weight:700;color:#64748b">رفع + دمج + تشفير + حفظ على جيتهاب</div></div>'+
        ...
        document.getElementById('ez-admin-encrypt-push').onclick=async function(){
            ...
            try{
                ...
                if(mergeMode==='merge'){
                    ...
                    /* ② دمج — الموجود أولاً ثم إضافة الجديد بدون تكرار */
                    var mergedMap={};
                    existingDB.forEach(function(d){
                        if(d.code) mergedMap[d.code.toLowerCase()]=d;
                    });
                    ...
                    st.textContent='🔀 تم الدمج: '+existingDB.length+' موجود + '+addedCount+' جديد = '+finalDB.length+' إجمالي';
                } else {
                    /* استبدال كامل */
                    finalDB=pendingDB;
                ...
                st.textContent=st.textContent+' — جاري التشفير والرفع...';
