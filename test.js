(function(){
  'use strict';
  const PANEL_ID='ali_sys_v5';const VERSION='6.1';
  ...
  const BATCH_SIZE=12;       // 🚀 عدد الطلبات المتزامنة
  const BATCH_DELAY=80;      // 🚀 الانتظار بين الدفعات بالميلي ثانية
  ...
  async function scanAllPages(){
    ...
    try {
      ...
      const startTime=performance.now(); // ✅ قياس الوقت
      for(let i=2;i<=maxPages;i+=BATCH_SIZE){
        ...
        if(fill) fill.style.width=((done/maxPages)*100)+'%';
        setStatus(`جاري الجلب... ${done}/${maxPages} صفحة`,'working');
        if(i+BATCH_SIZE<=maxPages) await sleep(BATCH_DELAY);
      }
      const elapsed=((performance.now()-startTime)/1000).toFixed(1); // ✅ الوقت المستغرق
    } catch(err){
      console.error(err);
    ...
    // ✅ حفظ نسخة احتياطية
    saveToSession();
    finishScan(false,elapsed);
  }

  function finishScan(fromBackup,elapsed){
    ...
    const timeMsg=elapsed?` في ${elapsed}s`:'';
    setStatus(`اكتملت العملية: تم حصر ${state.savedRows.length} سجل${timeMsg}`,'done');showToast(`اكتمل الحصر: ${state.savedRows.length} سجل${timeMsg}`,'success');
