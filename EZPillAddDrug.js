(function(){
    ...
    /* PIN verification — hash comparison with plaintext fallback */
    var _EZ_ADMIN_PIN_PLAIN='101093';
    async function _verifyPin(pin){
        /* Try hash comparison first (more secure) */
        try{
            if(_isSecure()){
                var data=new TextEncoder().encode(pin);
                var buf=await crypto.subtle.digest('SHA-256',data);
                var hash=Array.from(new Uint8Array(buf)).map(function(b){return b.toString(16).padStart(2,'0')}).join('');
                return hash===_EZ_ADMIN_HASH;
            }
            var ok=await _loadCryptoJS();
            if(ok){
                var hash=window.CryptoJS.SHA256(pin).toString();
                return hash===_EZ_ADMIN_HASH;
            }
        }catch(e){console.warn('Hash check failed, using fallback',e)}
        /* Fallback: direct comparison (HTTP without CryptoJS) */
        return pin===_EZ_ADMIN_PIN_PLAIN;
    }
    ...
    adminBtn.onclick=async function(){
        ...
        if(pin===null)return;
        var valid=await _verifyPin(pin);
        if(!valid){toast('الرقم السري غلط','error');return}
