// ═══════════════════════════════════════════════════════════════════
// مُنهي الطلبات v3.2 - نسخة التجميع الدقيق (بدون تغيير UI)
...
javascript:(function(){
  ...
  // ─── Dialog System (نفس كودك v3.1 بالمللي) ───
  ...
  function animNum(id,val){ const el=document.getElementById(id); if(!el)return; if(el.innerText!==String(val)){ el.innerText=val; el.style.animation='aliCountUp 0.4s'; } }

  // ─── Core Scan Logic (تم تحسين دقة الجمع هنا) ───
  function scanPage(curr, total) {
    ...
    // تجميع البيانات بدقة (Trim وتحقق من الـ '0')
    const rows = document.querySelectorAll('table tr');
    rows.forEach(row => {
      const cells = row.querySelectorAll('td');
      if(cells.length > 1){
        const invoiceText = cells[0].innerText.trim();
        // التحقق من أن رقم الفاتورة يبدأ بـ 0 وله طول منطقي (أكبر من 5)
        if(invoiceText.startsWith('0') && invoiceText.length > 5){
          if(!state.visitedSet.has(invoiceText)){
            state.visitedSet.add(invoiceText);
            ...
            state.savedRows.push({id:invoiceText, onl:cells[1].innerText.trim(), node:clone, st:isR?'received':(isP?'packed':'other'), hid:hId});
          }
        }
      }
    });

    // رصد ديناميكي للصفحات (مهم جداً للوصول لآخر السيستم)
    ...
    if (pNodes.length > 0) {
      ...
      if (highest > total) {
        total = highest;
        document.getElementById('p_lim').value = highest;
      }
    }
    ...
    if(curr < total){
      ...
      if(nxt){
        nxt.click();
        // تأخير 11 ثانية لضمان تحميل بيانات الصفحة الجديدة بالكامل
        setTimeout(() => scanPage(curr + 1, total), 11000);
      } else { finishScan(); }
    } else {
      // انتظار أخير ثانية واحدة قبل الإنهاء لضمان جمع آخر صفحة
      setTimeout(finishScan, 1000);
    }
  }

  // ─── بقية الدوال (نفس v3.1) ───
  function finishScan(){
    ...
    const recCount=updateStats(); setStatus(`تم! — ${state.savedRows.length} طلب`,'done');
    // هنا يظهر منطق البحث والفتح الأصلي v3.1... (لا تغيير)
    renderFinalUI(recCount, tbody);
  }

  function renderFinalUI(recCount, tbody) {
    const mainBody=document.getElementById('ali_main_body');
    mainBody.innerHTML=`<div style="margin-bottom:16px"><div style="position:relative;margin-bottom:8px"><span style="position:absolute;left:14px;top:50%;transform:translateY(-50%);font-weight:900;color:#cbd5e1;font-family:monospace;z-index:1">0</span><input type="text" id="ali_sI" placeholder="أدخل الأرقام..." style="width:100%;padding:12px 16px 12px 32px;border:2px solid #e2e8f0;border-radius:12px;font-family:'Tajawal',monospace;direction:ltr;text-align:left;font-weight:700"></div><input type="text" id="ali_sO" placeholder="ERX..." style="width:100%;padding:12px;margin-bottom:10px;border:2px solid #e2e8f0;border-radius:12px;direction:rtl"></div><div style="background:#f8fafc;padding:14px;border-radius:14px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center"><span style="font-size:14px;font-weight:700">فتح المفتوح:</span><input type="number" id="ali_open_count" value="${recCount}" style="width:60px;text-align:center;font-weight:900;border:2px solid #3b82f6;border-radius:8px"></div><button id="ali_btn_open" style="width:100%;padding:14px;background:linear-gradient(135deg,#059669,#10b981);color:white;border:none;border-radius:12px;font-weight:800;cursor:pointer">📂 فتح ومعالجة Received</button><button id="ali_btn_export" style="width:100%;padding:14px;background:linear-gradient(135deg,#d97706,#f59e0b);color:white;border:none;border-radius:12px;font-weight:800;cursor:pointer;margin-top:8px">📥 تصدير Packed</button>`;
    ...
    document.getElementById('ali_btn_export').onclick=async()=>{
      const p = state.savedRows.filter(r=>r.st==='packed');
      const res = await showExportDialog(p);
      if(res.action==='download' && res.orders.length) downloadSplitFiles(res.orders, res.pharmacyCode);
    };
  }
