javascript:(function(){
  ...
  // ═══════════════════════════════════════════════════════════════════
  // EZ-PILL PRO v5.0 - نسخة محسّنة بالكامل
  ...
  // │  تحسينات v5.0                                                 │
  // ├─────────────────────────────────────────────────────────────────┤
  // │  ✅ استخدام fetch بدل التنقل — مفيش reload أصلاً             │
  // │  ✅ كشف URL الصفحات تلقائي من الـ pagination links            │
  // │  ✅ Fallback ذكي: fetch → click » → click رقم → URL يدوي     │
  // │  ✅ Retry تلقائي لو fetch فشل (3 محاولات)                     │
  // │  ✅ شغال مع أي عدد صفحات بدون حد                             │
  // │  ✅ كل مميزات v4.1 موجودة + الإصلاحات                        │
  // └─────────────────────────────────────────────────────────────────┘
  ...
  const state = {
    ...
    openedCount: 0,
    tbody: null,
    pageUrlPattern: null,   // نمط URL الصفحات
    abortController: null   // للإلغاء
  };

  // ═══════════════════════════════════════════
  //  كشف نمط URL الصفحات
  // ═══════════════════════════════════════════
  function detectPageUrlPattern() {
    // طريقة 1: من href في روابط الـ pagination
    var paginationLinks = document.querySelectorAll('.pagination a[href], .pager a[href], nav a[href], [class*="page"] a[href]');
    for (var i = 0; i < paginationLinks.length; i++) {
      var href = paginationLinks[i].getAttribute('href');
      var text = paginationLinks[i].innerText.trim();
      var pageNum = parseInt(text);
      if (!isNaN(pageNum) && pageNum > 0 && href && href.length > 1) {
        // استبدال رقم الصفحة بـ placeholder
        var pattern = href.replace(new RegExp('([?&/=])' + pageNum + '([&/#]|$)'), '$1{{PAGE}}$2');
        if (pattern !== href) {
          return { type: 'href', pattern: pattern, sample: href, pageNum: pageNum };
        }
      }
    }

    // طريقة 2: من onclick في روابط الـ pagination
    var allPagEls = document.querySelectorAll('.pagination a, .pagination li a, .pagination span, [class*="page"] a');
    for (var j = 0; j < allPagEls.length; j++) {
      var onclick = allPagEls[j].getAttribute('onclick') || '';
      var txt = allPagEls[j].innerText.trim();
      var pNum = parseInt(txt);
      if (!isNaN(pNum) && pNum > 0 && onclick) {
        // حفظ onclick pattern
        var onclickPattern = onclick.replace(new RegExp(pNum.toString(), 'g'), '{{PAGE}}');
        if (onclickPattern !== onclick) {
          return { type: 'onclick', pattern: onclickPattern, original: onclick, pageNum: pNum };
        }
      }
    }

    // طريقة 3: من الـ URL الحالي
    var url = window.location.href;
    var pageMatch = url.match(/([?&]page=)(\d+)/i) ||
                    url.match(/([?&]p=)(\d+)/i) ||
                    url.match(/(\/page\/)(\d+)/i);
    if (pageMatch) {
      return { type: 'url', pattern: url.replace(pageMatch[0], pageMatch[1] + '{{PAGE}}'), pageNum: parseInt(pageMatch[2]) };
    }
  ...
  var pNodes = Array.from(document.querySelectorAll('.pagination a, .pagination li, .pagination span, [class*="page"] a, [class*="page"] span'))
    .map(function(el) { return parseInt(el.innerText.trim()); })
    .filter(function(n) { return !isNaN(n) && n > 0; });
  var defaultPages = pNodes.length > 0 ? Math.max.apply(null, pNodes) : 1;
    ...
    '<div class="ali-inner">' +
      '<div style="background:linear-gradient(135deg,#1e3a5f,#0f2744);padding:20px 22px 18px;color:white;position:relative;overflow:hidden">' +
        ...
        '<div style="text-align:right;margin-top:4px;position:relative;z-index:1">' +
          '<span style="display:inline-block;background:rgba(59,130,246,0.2);color:#93c5fd;font-size:10px;padding:2px 8px;border-radius:6px;font-weight:700">v5.0 – FETCH MODE</span>' +
        '</div>' +
      '</div>' +
  ...
  // ═══════════════════════════════════════════
  //  استخراج البيانات من HTML (صفحة حالية أو محمّلة)
  // ═══════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════
  //  تحميل صفحة واحدة بـ fetch مع retry
  // ═══════════════════════════════════════════
  async function fetchPageHTML(url, retries) {
    retries = retries || 3;
    for (var attempt = 1; attempt <= retries; attempt++) {
      try {
        var response = await fetch(url, {
          credentials: 'include',  // مهم: يبعت الـ cookies
        ...
        if (e.name === 'AbortError') throw e; // تم الإلغاء
  ...
  // ═══════════════════════════════════════════
  //  بناء URL لصفحة معينة
  // ═══════════════════════════════════════════
  function buildPageUrl(pageNum) {
    var pattern = state.pageUrlPattern;
    if (!pattern) return null;

    if (pattern.type === 'href') {
      var url = pattern.pattern.replace('{{PAGE}}', pageNum);
      // لو relative URL
      if (url.startsWith('/')) {
    ...
    if (pattern.type === 'url') {
      return pattern.pattern.replace('{{PAGE}}', pageNum);
    }
  ...
  // ═══════════════════════════════════════════
  //  الحصول على URL الصفحة بأي طريقة متاحة
  // ═══════════════════════════════════════════
  function getPageUrl(pageNum) {
    // أولاً: حاول من النمط المكتشف
    var fromPattern = buildPageUrl(pageNum);
    if (fromPattern) return fromPattern;

    // ثانياً: حاول من روابط الـ pagination الموجودة
    var allLinks = document.querySelectorAll('.pagination a[href], .pager a[href], [class*="page"] a[href]');
    for (var i = 0; i < allLinks.length; i++) {
      if (allLinks[i].innerText.trim() == String(pageNum)) {
    ...
    // ثالثاً: حاول بناء URL يدوي من الحالي
    var currentUrl = window.location.href;
    // جرب تبديل page parameter
    if (currentUrl.match(/[?&]page=\d+/i)) {
      return currentUrl.replace(/([?&]page=)\d+/i, '$1' + pageNum);
    }
    if (currentUrl.match(/[?&]p=\d+/i)) {
      return currentUrl.replace(/([?&]p=)\d+/i, '$1' + pageNum);
    }
    // جرب إضافة page parameter
    var sep = currentUrl.indexOf('?') !== -1 ? '&' : '?';
    return currentUrl + sep + 'page=' + pageNum;
  }

  // ═══════════════════════════════════════════
  //  المسح الرئيسي بـ fetch
  // ═══════════════════════════════════════════
  var totalNoArgs = 0;
  ...
  async function scanAllPages(totalPages, isSync) {
    ...
    // الخطوة 1: اجمع من الصفحة الحالية أولاً
    ...
    var result1 = extractRowsFromDocument(document);
    totalNoArgs += result1.noArgs;
    updateStats();

    // الخطوة 2: حمّل باقي الصفحات بـ fetch
    var failedPages = [];
    for (var p = 2; p <= totalPages; p++) {
      ...
      setStatus(isSync
        ? 'مزامنة الصفحة ' + p + ' من ' + totalPages + '...'
        : 'تحليل الصفحة ' + p + ' من ' + totalPages + ' ... تم رصد ' + state.savedRows.length + ' طلب',
        isSync ? 'sync' : 'working');

      var pageUrl = getPageUrl(p);

      try {
        var doc = await fetchPageHTML(pageUrl);
        var result = extractRowsFromDocument(doc);
        totalNoArgs += result.noArgs;
        updateStats();

        // تأخير بسيط بين الطلبات عشان ما نثقلش السيرفر
        ...
        failedPages.push(p);
        showToast('فشل تحميل الصفحة ' + p + ' — جاري التخطي', 'warning');
        // تأخير أطول قبل المحاولة التالية
        await new Promise(function(r) { setTimeout(r, 1500); });
      }
    }

    // الخطوة 3: إعادة محاولة الصفحات الفاشلة
    if (failedPages.length > 0 && state.isProcessing) {
      showToast('إعادة محاولة ' + failedPages.length + ' صفحة فشلت...', 'info');
      for (var fi = 0; fi < failedPages.length; fi++) {
        ...
        try {
          ...
          var retryDoc = await fetchPageHTML(getPageUrl(fp));
          var retryResult = extractRowsFromDocument(retryDoc);
  ...
  document.getElementById('ali_start').addEventListener('click', async function() {
    ...
    // كشف نمط الـ URL
    state.pageUrlPattern = detectPageUrlPattern();
    if (state.pageUrlPattern) {
      showToast('تم كشف نمط الصفحات ✅', 'success');
    } else {
      showToast('تعذر كشف النمط — سيتم تخمين URLs', 'warning');
    }
