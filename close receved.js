...
const VERSION = '3.8'; // تم رفع الإصدار لتمييز النسخة المحسنة
...
lastPageSignature: null // بصمة أقوى: أول 3 صفوف
};
...
let r=0,p=0,d=0;
state.savedRows.forEach(x=>{if(x.st==='received')r++; if(x.st==='packed')p++; if(x.st==='processed')d++;});
document.getElementById('stat_rec').innerText=r;
document.getElementById('stat_pack').innerText=p;
document.getElementById('stat_done').innerText=d;
document.getElementById('stat_total').innerText=state.savedRows.length;
return r;
}
// ─── بصمة أقوى وأدق للكشف عن تغيير الجدول ───
function getPageSignature() {
const rows = Array.from(document.querySelectorAll('table tr')).slice(0, 4); // أول 4 صفوف (تخطي الهيدر إن وجد)
...
let attempts = 0;
const maxWait = 45000; // زيادة الانتظار إلى 45 ثانية لأقصى دقة
...
await new Promise(r => setTimeout(r, 1200)); // فحص كل 1.2 ثانية
attempts++;
}
// إذا فشل الكشف تماماً، نعطي محاولة أخيرة للجمع (للحالات النادرة)
if (Date.now() - startTime >= maxWait) {
showToast('⚠️ تحذير: لم يتم اكتشاف تغيير الجدول، جاري جمع قسري', 'warning');
}
let foundThisPage = 0;
...
updateStats();
if (count === 0 && curr > 1) {
showToast(⚠️ صفحة ${curr}: لا بيانات جديدة، قد يكون التنقل فشل, 'warning');
} else if (count > 0) {
showToast(✅ صفحة ${curr}: تم جمع ${count} طلب جديد, 'success');
}
if(curr < total){
let nxt = null;
// بحث أوسع وأدق عن زر الصفحة التالية
...
if (el.offsetParent !== null) nxt = el; // مرئي فقط
}
});
if(nxt){
nxt.scrollIntoView({behavior: "smooth", block: "center"});
setTimeout(() => nxt.click(), 800);
setTimeout(() => scan(curr + 1, total), 6000); // انتظار أطول بعد الكليك لضمان التحميل
} else {
showToast('🔚 لا يوجد زر صفحة تالية، تم الإنهاء', 'warning');
finish();
}
} else {
finish();
