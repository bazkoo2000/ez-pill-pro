// ═══════════════════════════════════════════════════════════════════
// مُنهي الطلبات v3.9 - نسخة محرك الدقة والسرعة
...
javascript:(function(){
  ...
  const state = {
    ...
    startTime: null,
    isProcessing: false,
    lastPageSignature: null // البصمة الذكية
  };

  window.name = "ali_main_window";

  // ─── Toast & Dialog (تصميم علي الباز الأصلي) ───
    ...
    <div class="ali-inner">
      <div style="background:linear-gradient(135deg,#1e3a5f,#0f2744);padding:20px;color:white;">
        <div style="display:flex;justify-content:space-between;align-items:center">
          ...
          <h3 style="margin:0;font-size:20px;font-weight:900">مُنهي الطلبات</h3>
        </div>
        <div style="text-align:right;margin-top:4px"><span style="background:rgba(59,130,246,0.2);color:#93c5fd;font-size:10px;padding:2px 8px;border-radius:6px;font-weight:700">v3.9 Pro</span></div>
      </div>
      <div style="padding:22px" id="ali_body">
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:20px;text-align:center">
          <div style="background:#f8fafc;padding:10px;border-radius:14px"><div id="stat_rec" style="font-size:20px;font-weight:900;color:#10b981">0</div><div style="font-size:9px">Received</div></div>
          <div style="background:#f8fafc;padding:10px;border-radius:14px"><div id="stat_pack" style="font-size:20px;font-weight:900;color:#f59e0b">0</div><div style="font-size:9px">Packed</div></div>
          <div style="background:#f8fafc;padding:10px;border-radius:14px"><div id="stat_done" style="font-size:20px;font-weight:900;color:#3b82f6">0</div><div style="font-size:9px">المنجز</div></div>
          <div style="background:#f8fafc;padding:10px;border-radius:14px"><div id="stat_total" style="font-size:20px;font-weight:900;color:#8b5cf6">0</div><div style="font-size:9px">إجمالي</div></div>
        </div>
        <div id="ali_main_body">
          <div style="background:#f8fafc;padding:16px;border-radius:16px;margin-bottom:16px;border:1px solid #f1f5f9">
            <div style="display:flex;justify-content:space-between;align-items:center">
              <span style="font-size:13px;font-weight:700">📄 صفحات الفحص</span>
              <input type="number" id="p_lim" value="10" style="width:50px;text-align:center;font-weight:800;border:2px solid #e2e8f0;border-radius:8px">
            </div>
          </div>
          <div id="status-msg" style="padding:10px;background:#f0fdf4;color:#15803d;border-radius:12px;margin-bottom:16px;font-size:13px;font-weight:600;text-align:center">✅ جاهز للبدء</div>
          <button id="ali_start" style="width:100%;padding:14px;border:none;border-radius:14px;cursor:pointer;font-weight:800;background:linear-gradient(135deg,#1e40af,#3b82f6);color:white;">⚡ بدء المعالجة الذكية</button>
        </div>
  ...
  // ─── محرك الدقة: منطق البصمة المدمج ───
  function getPageSignature() {
    const rows = Array.from(document.querySelectorAll('table tr')).slice(0, 4);
    if (rows.length < 2) return null;
    return rows.map(row => row.innerText.trim()).join('|||');
  }

  async function collectWithPrecision() {
    let startTime = Date.now();
    // الانتظار حتى يتغير الجدول عن البصمة القديمة (حد أقصى 15 ثانية للتجميع السريع)
    while (Date.now() - startTime < 15000) {
      let currentSig = getPageSignature();
      if (currentSig && currentSig !== state.lastPageSignature) {
        state.lastPageSignature = currentSig;
        break;
      }
      await new Promise(r => setTimeout(r, 800)); // فحص كل 800 مللي ثانية للسرعة
    }
  ...
  async function scan(curr, total) {
    ...
    const count = await collectWithPrecision();
    updateStats();
    ...
    if(curr < total){
      ...
      document.querySelectorAll('.pagination a, .pagination li a, .pagination span, a, button').forEach(el => {
        ...
        if(t == String(curr + 1) || t.includes('»') || t.includes('>') || t.includes('Next')){
          if (el.offsetParent !== null) nxt = el;
        }
      });
      if(nxt){
        nxt.click();
        // العودة للفحص فوراً مع ترك بصمة الصفحة تتغير
        setTimeout(() => scan(curr + 1, total), 1500);
      } else { finish(); }
    } else { finish(); }
  }
  ...
  function finish() {
    ...
    showToast(`🎉 تم التجميع بدقة متناهية: ${state.savedRows.length} طلب`,'success');
  }
