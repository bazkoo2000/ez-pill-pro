...
var CHANGELOG={
  ...
  '136.9':{
    ...
    features:[
      ...
      {icon:'✅',text:'الإصلاح: تعريف startDateStr و addDays قبل استخدامهم في الـ confirm'},
  ...
  '136.8':{
    title:'ez_pill Jvm - تحديث الأوقات والإصدار 🕐',
    features:[
      {icon:'🏷️',text:'تغيير الاسم إلى ez_pill Jvm'},
      {icon:'📅',text:'End Date -1 يوم تلقائياً لكل صف بعد المعالجة'},
      {icon:'🕐',text:'على الريق 8:00 | بعد الفطار 9:00 | صباحا 9:00 | ظهرا 12:00 | قبل الغداء 13:00 | بعد الغداء 14:00 | عصرا 14:00 | المغرب 18:00 | قبل العشاء 20:00 | بعد العشاء 21:00 | مساءا 21:00 | قبل النوم 22:00'}
    ]
  },
  '136.7':{
...
function _scanPackSizeWarnings(dialogM,dialogT){
  ...
  for(var i=0;i<rows.length;i++){
    ...
    if(cb&&!cb.checked){console.log('PACK ROW '+i+': unchecked, skip');continue;}
    var itemCode=ci>=0?(_ezGet(tds[ci]).match(/\d+/)||[''])[0]:'';
    if(itemCode&&fixedSizeCodes&&fixedSizeCodes[itemCode]){console.log('PACK ROW '+i+': fixed code '+itemCode+', skip');continue;}
    if(itemCode&&weeklyInjections.indexOf(itemCode)>-1){console.log('PACK ROW '+i+': weekly '+itemCode+', skip');continue;}
    var itemName=nmi>=0?_ezGet(tds[nmi]):'';
    if(!itemName){console.log('PACK ROW '+i+': no name');continue;}
    if(isNonTabletItem(itemName)){console.log('PACK ROW '+i+': non-tablet: '+itemName);continue;}
    var noteText='';
  ...
  for(var j=0;j<allItems.length;j++){
    ...
    /* Only flag as 28-based if effDays actually falls short of 30
       e.g. 56 pack ÷ 1/day = 56 days → covers 30, no problem
            56 pack ÷ 2/day = 28 days → less than 30, flag it */
...
/* ══════════════════════════════════════════
   ADD DRUG TO TABLE
   ══════════════════════════════════════════ */
window.ezOpenAddDrug=function(){
  var existing=document.getElementById('ez-adddrug-panel');
  if(existing){existing.remove();return;}
  if(!window._ezAddDrugDB) window._ezAddDrugDB=[];
  var panel=document.createElement('div');
  panel.id='ez-adddrug-panel';
  panel.style.cssText='position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:999999;width:380px;max-width:95vw;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 24px 80px rgba(99,102,241,0.18),0 4px 16px rgba(0,0,0,0.08);border:2px solid rgba(129,140,248,0.15);font-family:Cairo,sans-serif;animation:dialogEnter 0.4s cubic-bezier(0.16,1,0.3,1)';
  var html='';
  html+='<div style="height:3px;background:linear-gradient(90deg,#818cf8,#a78bfa,#818cf8);background-size:200% 100%;animation:barShift 4s ease infinite"></div>';
  html+='<div style="padding:16px 20px 12px;display:flex;align-items:center;gap:10px;border-bottom:1px solid rgba(129,140,248,0.08);cursor:move">';
  html+='<div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(145deg,#818cf8,#6366f1);display:flex;align-items:center;justify-content:center;font-size:16px;box-shadow:0 4px 14px rgba(99,102,241,0.25)">➕</div>';
  html+='<div style="flex:1"><div style="font-size:15px;font-weight:900;color:#1e1b4b">إضافة صنف للجدول</div>';
  html+='<div id="ez-adddrug-count" style="font-size:10px;font-weight:700;color:#6366f1">'+(window._ezAddDrugDB.length?window._ezAddDrugDB.length+' صنف محمّل':'ارفع ملف الأصناف أولاً')+'</div></div>';
  html+='<button onclick="document.getElementById(\'ez-adddrug-panel\').remove()" style="width:28px;height:28px;border-radius:8px;border:1px solid rgba(129,140,248,0.12);background:rgba(129,140,248,0.05);color:#818cf8;cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center">×</button>';
  html+='</div>';
  html+='<div style="padding:14px 20px">';
  /* File upload */
  html+='<div style="margin-bottom:12px"><div style="font-size:11px;font-weight:800;color:#4a5568;margin-bottom:5px">📂 رفع ملف Excel / CSV</div>';
  html+='<input id="ez-adddrug-file" type="file" accept=".xlsx,.xls,.csv" style="width:100%;padding:8px;border:2px dashed rgba(129,140,248,0.2);border-radius:10px;background:rgba(241,245,249,0.5);font-size:12px;font-family:Cairo,sans-serif;box-sizing:border-box" /></div>';
  /* Search */
  html+='<div style="margin-bottom:12px;position:relative"><div style="font-size:11px;font-weight:800;color:#4a5568;margin-bottom:5px">🔍 بحث بالاسم أو الكود</div>';
  html+='<input id="ez-adddrug-q" type="text" placeholder="ابحث هنا..." style="width:100%;padding:10px 12px;border:1.5px solid rgba(129,140,248,0.15);border-radius:10px;font-size:13px;font-weight:700;font-family:Cairo,sans-serif;outline:none;box-sizing:border-box;direction:rtl" />';
  html+='<div id="ez-adddrug-suggestions" style="position:absolute;top:100%;left:0;right:0;background:#fff;border:1.5px solid rgba(129,140,248,0.15);border-radius:10px;max-height:200px;overflow-y:auto;display:none;z-index:1000000;box-shadow:0 8px 24px rgba(0,0,0,0.1);margin-top:4px"></div></div>';
  /* Add button */
  html+='<button id="ez-adddrug-btn" style="width:100%;padding:12px;border-radius:12px;border:none;background:linear-gradient(145deg,#818cf8,#6366f1);color:#fff;cursor:pointer;font-weight:800;font-size:14px;font-family:Cairo,sans-serif;box-shadow:0 4px 14px rgba(99,102,241,0.2);transition:all 0.3s">➕ إضافة الصنف</button>';
  /* Status */
  html+='<div id="ez-adddrug-status" style="margin-top:10px;padding:8px;border-radius:8px;background:rgba(241,245,249,0.5);font-size:11px;font-weight:700;color:#64748b;text-align:center;min-height:20px"></div>';
  html+='</div>';
  ...
  /* ── File upload handler ── */
  document.getElementById('ez-adddrug-file').onchange=function(){
    var f=this.files&&this.files[0];if(!f)return;
    var statusEl=document.getElementById('ez-adddrug-status');
    statusEl.textContent='⏳ جاري التحميل...';
    window._ezAddDrugParseFile(f,function(db){
      if(!db){statusEl.textContent='❌ خطأ في قراءة الملف';return;}
      window._ezAddDrugDB=db;
      statusEl.textContent='✅ تم تحميل '+db.length+' صنف';
      var countEl=document.getElementById('ez-adddrug-count');
      if(countEl) countEl.textContent=db.length+' صنف محمّل';
      document.getElementById('ez-adddrug-q').focus();
    });
  };

  /* ── Search suggestions ── */
  document.getElementById('ez-adddrug-q').oninput=function(){
    var q=this.value.trim().toLowerCase();
    var sug=document.getElementById('ez-adddrug-suggestions');
    sug.innerHTML='';
    if(q.length<1||!window._ezAddDrugDB.length){sug.style.display='none';return;}
    var matches=window._ezAddDrugDB.filter(function(it){return(it.name||'').toLowerCase().indexOf(q)>-1||(it.code||'').toLowerCase().indexOf(q)>-1;}).slice(0,10);
    if(!matches.length){sug.style.display='none';return;}
    for(var i=0;i<matches.length;i++){
      (function(item){
        var div=document.createElement('div');
        div.style.cssText='padding:10px 12px;cursor:pointer;border-bottom:1px solid rgba(0,0,0,0.04);direction:rtl;transition:background 0.2s';
        div.innerHTML='<div style="font-size:12px;font-weight:800;color:#1e1b4b">'+item.name+'</div><div style="font-size:10px;font-weight:700;color:#6366f1">كود: '+item.code+'</div>';
        div.onmouseover=function(){this.style.background='rgba(99,102,241,0.06)';};
        div.onmouseout=function(){this.style.background='';};
        div.onclick=function(){document.getElementById('ez-adddrug-q').value=item.name;sug.style.display='none';};
        sug.appendChild(div);
      })(matches[i]);
    }
  ...
  /* ── Add button ── */
  document.getElementById('ez-adddrug-btn').onclick=function(){window._ezDoAddDrug();};
  document.getElementById('ez-adddrug-q').onkeydown=function(e){if(e.key==='Enter')window._ezDoAddDrug();};
};

/* ── Add Drug: parse file ── */
window._ezAddDrugParseCSV=function(text){
  ...
  function splitLine(line){var out=[],cur='',q=false;for(var i=0;i<line.length;i++){var ch=line[i];if(ch==='"'){if(q&&line[i+1]==='"'){cur+='"';i++}else{q=!q}}else if(ch===sep&&!q){out.push(cur);cur=''}else{cur+=ch}}out.push(cur);return out.map(function(s){return s.replace(/^"|"$/g,'').trim()})}
  var head=splitLine(lines[0]).map(function(h){return h.toLowerCase()});
  var iName=-1,iCode=-1;
  for(var h=0;h<head.length;h++){if(head[h]==='name'||head[h].indexOf('name')>-1||head[h].indexOf('اسم')>-1)iName=h;if(head[h]==='code'||head[h].indexOf('code')>-1||head[h].indexOf('كود')>-1)iCode=h;}
  if(iName<0)iName=0;if(iCode<0)iCode=1;
  var db=[];
  for(var r=1;r<lines.length;r++){var cols=splitLine(lines[r]);var nm=(cols[iName]||'').trim();var cd=(cols[iCode]||'').trim();if(nm||cd)db.push({name:nm,code:cd});}
  return db;
};

window._ezAddDrugParseFile=function(file,cb){
  var ext=(file.name||'').toLowerCase();
  if(ext.endsWith('.csv')||ext.endsWith('.txt')){
    var fr=new FileReader();fr.onload=function(){cb(window._ezAddDrugParseCSV(String(fr.result||'')));};fr.readAsText(file);return;
  }
  /* Excel: load XLSX library */
  if(window.XLSX){window._ezAddDrugReadXLSX(file,cb);return;}
  var s=document.createElement('script');
  s.src='https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js';
  s.onload=function(){window._ezAddDrugReadXLSX(file,cb);};
  s.onerror=function(){cb(null);};
  document.head.appendChild(s);
};

window._ezAddDrugReadXLSX=function(file,cb){
  ...
  fr.onload=function(e){
    try{
      ...
      var head=(rows[0]||[]).map(function(x){return(x||'').toString().toLowerCase().trim()});
      var iName=-1,iCode=-1;
      for(var h=0;h<head.length;h++){if(head[h]==='name'||head[h].indexOf('name')>-1||head[h].indexOf('اسم')>-1)iName=h;if(head[h]==='code'||head[h].indexOf('code')>-1||head[h].indexOf('كود')>-1)iCode=h;}
      if(iName<0)iName=0;if(iCode<0)iCode=1;
      var db=[];
      for(var i=1;i<rows.length;i++){var r=rows[i]||[];var nm=(r[iName]||'').toString().trim();var cd=(r[iCode]||'').toString().trim();if(nm||cd)db.push({name:nm,code:cd});}
      cb(db);
    }catch(err){cb(null);}
  };
...
window._ezDoAddDrug=function(){
  var q=(document.getElementById('ez-adddrug-q').value||'').trim();
  var statusEl=document.getElementById('ez-adddrug-status');
  if(!q){statusEl.textContent='⚠️ اكتب اسم أو كود الصنف';return;}
  if(!window._ezAddDrugDB||!window._ezAddDrugDB.length){statusEl.textContent='⚠️ ارفع ملف الأصناف أولاً';return;}
  var ql=q.toLowerCase();
  var item=null;
  /* Exact match first */
  for(var i=0;i<window._ezAddDrugDB.length;i++){var it=window._ezAddDrugDB[i];if((it.code||'').toLowerCase()===ql||(it.name||'').toLowerCase()===ql){item=it;break;}}
  if(!item){for(var i=0;i<window._ezAddDrugDB.length;i++){var it=window._ezAddDrugDB[i];if((it.name||'').toLowerCase().indexOf(ql)>-1||(it.code||'').toLowerCase().indexOf(ql)>-1){item=it;break;}}}
  if(!item){statusEl.textContent='❌ مش موجود: '+q;return;}
  /* Add to table */
  var tb=_ezFindTable();if(!tb){statusEl.textContent='❌ الجدول مش موجود';return;}
  var hdr=tb.querySelector('tr');var hs=hdr.querySelectorAll('th,td');
  var iName=_ezIdx(hs,'name');if(iName<0)iName=_ezIdx(hs,'item');
  var iCode=_ezIdx(hs,'code');
  if(iName<0||iCode<0){statusEl.textContent='❌ أعمدة Name/Code مش موجودة';return;}
  var tbody=tb.tBodies&&tb.tBodies.length?tb.tBodies[0]:tb;
  var rows=[].slice.call(tbody.querySelectorAll('tr')).filter(function(r){return r.querySelectorAll('th').length===0;});
  if(!rows.length){statusEl.textContent='❌ مفيش صفوف في الجدول';return;}
  var base=rows[rows.length-1];var newRow=base.cloneNode(true);
  /* Clear the new row */
  newRow.removeAttribute('style');
  var inputs=newRow.querySelectorAll('input,textarea,select');
  for(var j=0;j<inputs.length;j++){var x=inputs[j];if(x.type==='checkbox'){x.checked=false;_ezFire(x);}else if(x.tagName==='SELECT'){x.selectedIndex=0;_ezFire(x);}else{x.value='';_ezFire(x);}}
  var tds=newRow.querySelectorAll('td');
  /* Check the checkbox if exists */
  if(tds[0]){var ck=tds[0].querySelector('input[type="checkbox"]');if(ck){ck.checked=true;_ezFire(ck);}}
  /* Set name and code */
  var nameCell=tds[iName];if(nameCell){var ni2=nameCell.querySelector('input,textarea');if(ni2){ni2.value=item.name;_ezFire(ni2);}else{nameCell.textContent=item.name;}}
  var codeCell=tds[iCode];if(codeCell){var ci2=codeCell.querySelector('input,textarea');if(ci2){ci2.value=item.code;_ezFire(ci2);}else{codeCell.textContent=item.code;}}
  /* Set dose=1, qty=1 */
  if(tds[3]){var d3=tds[3].querySelector('input,textarea,select');if(d3){d3.value='1';_ezFire(d3);}}
  if(tds[5]){var d5=tds[5].querySelector('input,textarea,select');if(d5){d5.value='1';_ezFire(d5);}}
  tbody.appendChild(newRow);
  newRow.scrollIntoView({block:'center'});
  statusEl.textContent='✅ تم إضافة: '+item.name;
  document.getElementById('ez-adddrug-q').value='';
  document.getElementById('ez-adddrug-suggestions').style.display='none';
  ezBeep('success');
};
...
window.ezSelect=function(el,type,val){
  ...
  try{_renderPackWarningBanner();}catch(e){console.error("PACK SELECT ERROR:",e);}
};
...
window.ezPasteNotes=function(){
  ...
  /* Build preview: match codes */
  var matches=[];var unmatched=[];
  for(var i=0;i<rows.length;i++){
    var tds=rows[i].querySelectorAll('td');
    if(tds.length<=Math.max(ni,ci)) continue;
    var code=(_ezGet(tds[ci]).match(/\d+/)||[''])[0];
  ...
  /* Store matches for _ezDoPasteNotes */
...
window._ezDoPasteNotes=function(){
  ...
  for(var m=0;m<matches.length;m++){
    ...
    var mt=matches[m];
    var inp=mt.tds[_ezIdx(mt.row.closest('table').querySelector('tr').querySelectorAll('th,td'),'note')];
    /* Safer: find note cell directly */
  ...
  ezBeep('success');
  /* Refresh pack warnings if dialog is open */
...
window.showWarnings=function(warnings,callback){
  ...
  /* ── "Apply All" global bar for days warnings ── */
  ...
  /* Mark outliers (different from majority) and sort: outliers first */
  var _outlierIdxs={};
  ...
  /* Build sorted render order: outlier days first, then rest */
  var _renderOrder=[];
  ...
  for(var _ri=0;_ri<_renderOrder.length;_ri++){
    ...
    if(w.type==='dose2'){
      ...
      /* Smart suggestion: 30→qty1, 60→qty2, 90→qty3 */
    ...
    if(w.editable){
      if(w.type==='ramadan_unclear'){
        ...
        /* Smart days UI: qty + size side by side */
...
/* ── Apply All Days Warnings at once from global inputs ── */
window.ezApplyAllDays=function(){
  ...
  for(var i=0;i<warningQueue.length;i++){
    ...
    /* Smart: calculate qty per item based on its own extracted days */
...
/* ── Apply All Warnings (bottom button) ── */
...
window._ezApplyRamadanSplit=function(daysLeft){
  ...
  rowsData.forEach(function(rd){
    ...
    if(rd.isRam){
      ...
      /* بعد الفطار → بعد الفطار عادي 09:00 (Arabic stays same, English changes) */
      if(origNote.indexOf('بعد الفطار')>-1||origNote.indexOf('After Iftar')>-1){
        newNote=origNote.replace(/⚡\s*/g,'').replace('After Iftar','After Breakfast');
      ...
      /* قبل الفطار → قبل الفطار عادي */
        ...
        newNote=origNote.replace(/⚡\s*/g,'').replace('Before Iftar','Before Breakfast');
...
window.ezCancelRamadanSplit=function(){
  ...
  /* إعادة إطلاق أحداث لكل خلايا الجدول */
  var fire=_ezFire;
...
window.ezRamadanToNormal=function(){
  ...
  if(ci>=0){
    ...
    Object.keys(groups).forEach(function(code){
      ...
      /* every بناءً على عدد الصفوف المدموجة */
      var mergedCount=g.length;
      var mergedEvery=mergedCount>=4?'6':mergedCount===3?'8':mergedCount===2?'12':'24';

      /* FIX: دمج النوتات - "بعد الفطار" + "بعد العشاء" → "بعد الفطار والعشاء" */
      ...
      if(notesList.length===2){
        if(isEnNotes){
          var p2en=notesList[1].replace(/^(Before|After)\s+/i,'');
          combinedNote=notesList[0]+' & '+p2en;
        } else {
        ...
        combinedNote=notesList[0]||(get(mtds[ni])||'').replace(/^⚡\s*/,'').trim();
      }
      ...
      /* qty لا يتغير - يبقى 1 كما وضعناه في وضع رمضان */
      for(var j=1;j<g.length;j++){if(g[j].parentNode)g[j].parentNode.removeChild(g[j]);}
    });
  }

  /* تحديث #fstartDate لأول يوم بعد رمضان */
...
function smartDoseRecognizer(note){
  ...
  /* قبل/بعد الاكل الثلاث أو before all 3 meals = 3 times */
  ...
  /* قبل/بعد/مع الاكل (ambiguous, no count specified) = 1 time only */
  ...
  /* يوميا / daily alone with no other frequency = 1 time */
...
function getTimeFromWords(w){
  ...
  var rules=[{test:/empty|stomach|ريق|الريق|على الريق|fasting/,time:'07:00'},{test:/قبل\s*(الاكل|الأكل|meal)|before\s*(meal|food)/,time:'08:00'},{test:/before.*bre|before.*fatur|before.*breakfast|before.*iftar|قبل.*فطر|قبل.*فطار|قبل.*فطور|قبل.*افطار/,time:'08:00'},{test:/after.*bre|after.*fatur|after.*breakfast|after.*iftar|بعد.*فطر|بعد.*فطار|بعد.*فطور|بعد.*افطار/,time:'09:00'},{test:/\b(morning|am|a\.m)\b|صباح|الصباح|صبح/,time:'09:00'},{test:/\b(noon|midday)\b|ظهر|الظهر/,time:'12:00'},{test:/before.*lun|before.*lunch|قبل.*غدا|قبل.*غداء|قبل.*غذا|قبل.*غذاء/,time:'13:00'},{test:/after.*lun|after.*lunch|بعد.*غدا|بعد.*غداء|بعد.*غذا|بعد.*غذاء/,time:'14:00'},{test:/\b(asr|afternoon|pm|p\.m)\b|عصر|العصر/,time:'15:00'},{test:/maghrib|مغرب|المغرب/,time:'18:00'},{test:/before.*din|before.*sup|before.*dinner|before.*asha|before.*suhoor|before.*sahoor|قبل.*عشا|قبل.*عشو|قبل.*عشاء|قبل.*سحور|قبل.*سحر/,time:'20:00'},{test:/after.*din|after.*sup|after.*dinner|after.*asha|after.*suhoor|after.*sahoor|بعد.*عشا|بعد.*عشو|بعد.*عشاء|بعد.*سحور|بعد.*سحر/,time:'21:00'},{test:/مساء|مسا|evening|eve/,time:'21:00'},{test:/bed|sleep|sle|نوم|النوم|hs|h\.s/,time:'22:00'}];
...
function processTable(m,t,autoDuration,enableWarnings,showPostDialog,ramadanMode){
  ...
  function continueProcessing(){
    ...
    try{
      ...
      /* Check if any non-fixed item has effDays < 30 (28-based) */
    ...
    setTimeout(function(){
      try{
        ...
        var tbl=_ezFindTable();
        if(!tbl) return;
        var h2=tbl.querySelector('tr'),hs2=h2.querySelectorAll('th,td');
        var endCol=_ezIdx(hs2,'end date');
        if(endCol<0) return;
        ...
        for(var r=1;r<rows.length;r++){
          ...
          var input=cells[endCol].querySelector('input');
          var dateVal=input?input.value:cells[endCol].textContent.trim();
...
<div class="ez-header">\
  ...
  <div class="ez-header-actions">\
    <button class="ez-btn-icon" onclick="window.ezOpenAddDrug()" title="إضافة صنف">➕</button>\
