(async function () {
  ...
  var css = '\
    ...
    #fd-bulk-panel .fd-log{margin-top:10px;padding:8px;background:#f5f5f5;border-radius:4px;max-height:180px;overflow-y:auto;font-size:11px;line-height:1.5;direction:ltr;word-break:break-all}\
  ...
  function toUTCDate(dateStr, isEnd) {
    ...
    if (isEnd) { d.setUTCDate(d.getUTCDate() - 1); return { date: d.getUTCFullYear() + "-" + String(d.getUTCMonth()+1).padStart(2,"0") + "-" + String(d.getUTCDate()).padStart(2,"0") + " 20:59:59.000000", timezone_type: 3, timezone: "UTC" }; }
    else { d.setUTCDate(d.getUTCDate() - 1); return { date: d.getUTCFullYear() + "-" + String(d.getUTCMonth()+1).padStart(2,"0") + "-" + String(d.getUTCDate()).padStart(2,"0") + " 21:00:00.000000", timezone_type: 3, timezone: "UTC" }; }
  }
  ...
  function renderIntakeGrid() {
    ...
    INTAKES.forEach(function (intake) {
      ...
      html += '<div class="fd-intake-item">' +
        ...
        '<span class="fd-intake-time">' + ts + '</span><span>' + intake.name + '</span>' +
        '<input type="number" class="fd-intake-dose" value="1" min="0.25" step="0.25" title="Dose" /></div>';
    });
    grid.innerHTML = html;
  }
  ...
  function recalcEndDate() {
    var s = document.getElementById("fd-start-date").value, q = parseInt(document.getElementById("fd-qty").value)||0, sz = parseInt(document.getElementById("fd-size").value)||0, t = q*sz;
  ...
  function recalcAddEndDate() {
    var s = document.getElementById("fd-add-start").value, q = parseInt(document.getElementById("fd-add-qty").value)||0, sz = parseInt(document.getElementById("fd-add-size").value)||0, t = q*sz;
  ...
  async function searchMedicine(query) {
    ...
    for (var ei = 0; ei < endpoints.length; ei++) {
      try {
        addLog("Trying endpoint " + (ei+1) + "...", "info");
        var resp = await apiFetch(endpoints[ei], {
          headers: { Authorization: "Bearer " + token, Accept: "application/json, text/plain, */*" },
        });
        addLog("Endpoint " + (ei+1) + " status: " + resp.status, resp.ok ? "ok" : "err");
        ...
        if (resp.ok) {
          ...
          if (results.length > 0) {
            var first = results[0];
            addLog("Sample: id=" + first.id + " code=" + first.code + " medicine_id=" + (first.medicine_id||"N/A"), "info");
          }

          if (results.length > 0) break;
        }
      } catch (e) { addLog("Endpoint " + (ei+1) + " error: " + e.message, "err"); }
    }

    var resultsEl = document.getElementById("fd-search-results");
    if (results.length === 0) {
      resultsEl.innerHTML = '<div style="padding:10px;color:#999;text-align:center;">No results found</div>';
    ...
    results.slice(0, 20).forEach(function (med) {
      // The search might return imported_medicine or medicine objects
      // We need to capture all relevant IDs
      var medData = {
        id: med.id,
        code: med.code,
        name: med.name,
      ...
      // If the result has a nested medicine object
      if (med.medicine && med.medicine.id) {
        medData.medicine_id = med.medicine.id;
        medData.imported_medicine_id = med.id;
      }

      html += '<div class="fd-search-item" data-med=\'' + JSON.stringify(medData).replace(/'/g, "&#39;") + '\'>' +
        '<strong>' + (med.code || "") + '</strong> - ' + med.name + '</div>';
    });
    ...
    resultsEl.querySelectorAll(".fd-search-item").forEach(function (item) {
      item.onclick = function () {
        ...
        document.getElementById("fd-selected-med").innerHTML =
          ...
          "<br/><small>medicine_id: " + selectedMedicine.medicine_id + " | imported_id: " + selectedMedicine.imported_medicine_id + "</small>";
        ...
        if (selectedMedicine.pills_per_pack > 1) {
          document.getElementById("fd-add-size").value = selectedMedicine.pills_per_pack;
          recalcAddEndDate();
        }
        addLog("Selected: " + selectedMedicine.code + " (med_id:" + selectedMedicine.medicine_id + " imp_id:" + selectedMedicine.imported_medicine_id + ")", "ok");
      };
    });
  }
  ...
  async function loadTreatments() {
    ...
    var token = getAuthToken();
    if (!token) { log("Token not found.", "err"); return false; }
    try {
      ...
      log("Found " + window._fd_allTreatments.length, "ok");
      renderList(); return true;
    } catch (err) { log("Error: " + err.message, "err"); return false; }
  }

  function renderList() {
    var ao = document.getElementById("fd-filter-active").checked;
    var list = window._fd_allTreatments || [];
    var filtered = list.filter(function (t) { return ao ? t.is_active !== false : true; });
    ...
    filtered.forEach(function (t, i) {
      var n = (t.medicine ? t.medicine.name : t.medicine_code) || "Unknown";
      ...
      html += '<div class="fd-treat-item"><input type="checkbox" class="fd-treat-cb" data-index="'+i+'" checked />' +
        '<div><strong style="font-size:12px;">'+n+'</strong><br/><span class="fd-tid">ID: '+t.id+'</span>' +
  ...
  document.getElementById("fd-update-btn").onclick = async function () {
    ...
    if (!ns||!ne) { log("Set both dates!", "err"); return; }
    var token = getAuthToken(); if (!token) { log("Token!", "err"); return; }
    var sel = document.querySelectorAll(".fd-treat-cb:checked");
    if (sel.length===0) { log("None selected", "err"); return; }
    ...
    clearLog("fd-log"); log("Updating "+sel.length+"...", "info");
    var ok=0,fail=0,tot=sel.length;
    for (var s=0;s<sel.length;s++) {
      ...
      try {
        var gr=await apiFetch(API_BASE+"/treatments/"+t.id,{headers:{Authorization:"Bearer "+token,Accept:"application/json, text/plain, */*"}});
        if (!gr.ok){log("["+tn+"] Load fail","err");fail++;continue;}
        var fd=await gr.json();
        fd.starts_at=ns;fd.ends_at=ne;fd.starts_at_8601=toUTCDate(ns,false);fd.ends_at_8601=toUTCDate(ne,true);
        var sp=ns.split("-"),sdd=new Date(Date.UTC(parseInt(sp[0]),parseInt(sp[1])-1,parseInt(sp[2])));
        sdd.setUTCDate(sdd.getUTCDate()-1);sdd.setUTCHours(21,0,0,0);fd.startsAtLocal=sdd.toISOString();
        ...
        if (pr.ok){log("("+(s+1)+"/"+tot+") ["+tn+"] Done","ok");ok++;}
        else{log("["+tn+"] Error "+pr.status,"err");fail++;}
  ...
  document.getElementById("fd-add-btn").onclick = async function () {
    ...
    if (!selectedMedicine.medicine_id && !selectedMedicine.id) { addLog("Medicine ID is missing! Search again.", "err"); return; }
    ...
    // Build payload matching the structure from a successful update
    var medicineId = selectedMedicine.medicine_id || selectedMedicine.id;
    var importedMedicineId = selectedMedicine.imported_medicine_id || selectedMedicine.id;

    var treatmentData = {
      medicine_code: selectedMedicine.code,
      medicine_id: medicineId,
      imported_medicine_id: importedMedicineId,
      ...
      external_id: null,
      medicine_family_id: null,
      previous_treatment_id: null,
      container_days_duration: null,
      quantity_in_container: null,
      doses_number_for_quantity_in_container: null,
      container_unit_measure: null,
      unit_measure_take: null,
      ws_revision_date_at_8601: null,
      ws_next_delivery_date_at_8601: null,
      configs: configs,
      treatment_configs: configs.map(function (c) {
        var parts = c.first_take.split(" ");
        var timeParts = parts[1].split(":");
        return {
          first_take_8601: {
            date: startDate + " " + String(parseInt(timeParts[0])).padStart(2,"0") + ":" + String(parseInt(timeParts[1])).padStart(2,"0") + ":00.000000",
            timezone_type: 3,
            timezone: "UTC"
          },
          days_to_take: null,
          minutes_interval: c.minutes_interval,
          range_to_take: null,
          dose: c.dose,
          days_in_months: null,
          month_ids: [],
    ...
    addLog("Payload medicine_id: " + medicineId, "info");
    addLog("Payload imported_medicine_id: " + importedMedicineId, "info");
    addLog("Payload medicine_code: " + selectedMedicine.code, "info");
    ...
    if (!confirm("Add treatment?\n" + selectedMedicine.code + " - " + selectedMedicine.name +
      "\nmed_id: " + medicineId + " | imp_id: " + importedMedicineId +
      "\nStart: " + startDate + " | End: " + endDate +
      "\nDoses: " + intakeChecks.length + " | " + freqLabel)) return;
    ...
    try {
      ...
      addLog("Response " + resp.status + ": " + respText.substring(0, 300), resp.ok ? "ok" : "err");
