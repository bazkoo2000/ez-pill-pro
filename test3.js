javascript:(function(){
  ...
  const state = { savedRows:[], visitedSet:new Set(), isProcessing:false, isSyncing:false, openedCount:0, tbody:null, noNewStreak:0, lastSearchKey: '', currentOffset: 0 };
    ...
    try{
      ...
      if(orders1&&orders1.length>0){
        ...
        for(var i1=0;i1<orders1.length;i1++){
          var inv1=orders1[i1].Invoice||'';
          if(inv1.length>3&&!state.visitedSet.has(inv1)){
            state.visitedSet.add(inv1);
            var rd1=buildOrderRow(orders1[i1],tpl);
            if(!rd1.hasArgs)nac1++;
            state.savedRows.push(rd1);
          }
        }
        totalNoArgs+=nac1;
      }
              ...
              fetchPageOrders(pageNum,cs).then(function(data){
                ...
                for(var i=0;i<orders.length;i++){
                  ...
                  if(inv.length>3&&!state.visitedSet.has(inv)){
                    ...
                    var rd=buildOrderRow(orders[i],tpl);
                    if(!rd.hasArgs)nac++;
                    state.savedRows.push(rd);
                  }
                }
                totalNoArgs+=nac;
  ...
  async function smartSync(){
    ...
    try{
      ...
      if(remIds.length===0&&newR.length===0)sp.push('لا تغييرات');
  ...
  function buildSearchUI(){var da=document.getElementById('ali_dynamic_area');
    da.innerHTML='<div class="ios-grp" style="padding:14px 16px"><div style="position:relative;margin-bottom:8px"><span style="position:absolute;left:12px;top:50%;transform:translateY(-50%);font-size:15px;font-weight:900;color:'+IOS.muted+';pointer-events:none;font-family:monospace">0</span><input type="text" id="ali_sI" class="ios-input" placeholder="أدخل الأرقام بعد الـ 0..." style="padding-left:30px;direction:ltr;text-align:left;letter-spacing:1px;font-family:monospace;font-weight:700"></div><div style="position:relative;margin-bottom:8px"><span style="position:absolute;right:12px;top:50%;transform:translateY(-50%);font-size:13px;pointer-events:none">🔗</span><input type="text" id="ali_sO" class="ios-input" placeholder="بحث برقم الطلب (ERX)..." style="padding-right:36px;direction:rtl"></div><div style="position:relative"><span style="position:absolute;right:12px;top:50%;transform:translateY(-50%);font-size:13px;pointer-events:none">📑</span><input type="number" id="ali_maxT" class="ios-input" placeholder="عدد التابات المُراد فتحها (اختياري)..." style="padding-right:36px;direction:rtl"></div></div><div id="ali_search_count" style="font-size:11px;color:'+IOS.muted+';text-align:center;font-weight:600;padding:2px 0 10px">عرض '+state.savedRows.length+' من '+state.savedRows.length+' نتيجة</div><button id="ali_btn_open" class="ios-btn ios-success" style="margin-bottom:8px;opacity:0.7;cursor:not-allowed">⚡ ابحث أولاً ثم افتح المطابق</button><button id="ali_btn_sync" class="ios-btn ios-ghost">🔄 مزامنة ذكية</button>';
    var sI=document.getElementById('ali_sI'),sO=document.getElementById('ali_sO'),maxT=document.getElementById('ali_maxT'),sc=document.getElementById('ali_search_count'),ob=document.getElementById('ali_btn_open'),cm=[];
    function fr(){
      var ri=sI.value.trim();var is=ri!==''?'0'+ri:'';var os=sO.value.trim().toLowerCase();
      var currentKey = is + '|' + os;
      if(state.lastSearchKey !== currentKey) { state.currentOffset = 0; state.lastSearchKey = currentKey; }
      state.tbody.innerHTML='';var sh=0;cm=[];var hf=is!==''||os!=='';
      for(var i=0;i<state.savedRows.length;i++){
        var rw=state.savedRows[i];var mi=is!==''&&rw.id.startsWith(is);var mo=os!==''&&rw.onl.toLowerCase().indexOf(os)!==-1;var s=hf?(mi||mo):true;
        if(s){state.tbody.appendChild(rw.node);sh++;if(hf)cm.push(rw)}
      }
      sc.innerText='عرض '+sh+' من '+state.savedRows.length+' نتيجة';updateStats(sh);
      if(hf&&cm.length>0){
        var available = cm.filter(function(r, idx){return r.args!==null && idx >= state.currentOffset});
        var limit = parseInt(maxT.value) || available.length;
        var toOpenCount = Math.min(limit, available.length);
        ob.innerHTML='⚡ فتح المطابق ('+toOpenCount+' طلب) ' + (state.currentOffset > 0 ? '[تبدأ من '+ (state.currentOffset+1) +']' : '');
        ob.style.opacity='1';ob.style.cursor='pointer'
      }else if(hf&&cm.length===0){ob.innerHTML='⚡ لا توجد نتائج';ob.style.opacity='0.5';ob.style.cursor='not-allowed'}else{ob.innerHTML='⚡ ابحث أولاً ثم افتح المطابق';ob.style.opacity='0.7';ob.style.cursor='not-allowed'}
      sI.className='ios-input'+(ri.length>0?(sh>0?' match':' nomatch'):'');sO.className='ios-input'+(os.length>0?(sh>0?' match':' nomatch'):'');
    }
    var df=debounce(fr,150);sI.addEventListener('input',df);sO.addEventListener('input',df);maxT.addEventListener('input',df);
    ob.addEventListener('click',async function(){
      var ri=sI.value.trim();var os=sO.value.trim().toLowerCase();var hf=ri!==''||os!=='';
      if(!hf){showToast('ابحث أولاً برقم الفاتورة أو رقم الطلب!','warning');sI.focus();return}
      
      var opAll = cm.filter(function(r){return r.args!==null});
      if(state.currentOffset >= opAll.length) { state.currentOffset = 0; showToast('بدء الفتح من البداية مرة أخرى', 'info'); }
      
      var op = opAll.slice(state.currentOffset);
      var limit = parseInt(maxT.value);
      if(!isNaN(limit) && limit > 0) op = op.slice(0, limit);

      var sk=cm.length-opAll.length;
      if(op.length===0){showToast('لا توجد المزيد من الطلبات لفتحها!', 'warning'); return;}
      
      ob.disabled=true;ob.style.opacity='0.6';ob.style.cursor='not-allowed';
      var opened=0,failed=0;var base=window.location.origin+"/ez_pill_web/getEZPill_Details";
      
      for(var idx=0;idx<op.length;idx++){
        var it=op[idx];
        ...
        try{var w=window.open(url,"_blank");if(w){opened++;state.openedCount++;window.focus();try{w.blur()}catch(e){}}else failed++}catch(e){failed++}
        ob.innerHTML='🚀 جاري الفتح ('+(idx+1)+'/'+op.length+')';
        setStatus('فتح '+(idx+1)+' من '+op.length+': '+(it.onl||it.id),'working');
        updateStats();
        if(idx<op.length-1)await new Promise(function(r){setTimeout(r,1200)});
      }
      
      state.currentOffset += opened;
      showToast('تم فتح '+opened+' طلب بنجاح', 'success');
      setStatus('تم فتح '+opened+' — الإجمالي المفتوح: '+state.openedCount,'done');
      ob.disabled=false; fr();
    });
