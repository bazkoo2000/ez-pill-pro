...
window.ezShowDoses=function(){
  ...
  rows.forEach(function(r){
    ...
    if(tds.length>Math.max(ni,nmi)){
      ...
      var rawNote=getVal(tds[ni]);
      var note=cleanN(rawNote);
      ...
      if(name&&note) items.push({name:name,note:note,isDup:isDup,code:code,rowRef:r,noteCell:tds[ni]});
    }
  });
  if(items.length===0){window.ezShowToast('لا توجد بيانات جرعات','info');return;}
  window._ezDoseItems=items;
  var html='';
  html+='<div class="ez-doses-header"><div class="ez-logo-group"><div class="ez-doses-logo">📋</div><div class="ez-title-block"><div class="ez-doses-title">جدول الجرعات</div><div class="ez-sub-info"><span class="ez-items-count">📦 '+items.length+' صنف</span></div></div></div><div style="display:flex;gap:6px;align-items:center"><button onclick="window._ezApplyDoseEdits()" style="height:32px;padding:0 14px;border:none;border-radius:9px;font-size:11px;font-weight:800;cursor:pointer;font-family:Cairo,sans-serif;color:#fff;background:linear-gradient(145deg,#10b981,#059669);box-shadow:0 3px 10px rgba(16,185,129,0.2)">✅ تطبيق التعديلات</button><button class="ez-btn-icon" onclick="window.ezCloseDoses()">×</button></div></div>';
  html+='<div class="ez-doses-body">';
  html+='<div class="ez-dose-header-row"><div class="ez-dose-num">#</div><div class="ez-dose-name">اسم الصنف</div><div class="ez-dose-note">الجرعة (قابلة للتعديل)</div></div>';
  for(var i=0;i<items.length;i++){
    var dupClass=items[i].isDup?' ez-dose-item-dup':'';
    var noteEsc=items[i].note.replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    html+='<div class="ez-dose-item'+dupClass+'">';
    html+='<div class="ez-dose-num">'+(i+1)+'</div>';
    html+='<div class="ez-dose-name">'+items[i].name+'</div>';
    html+='<div class="ez-dose-note" style="padding:4px 8px">';
    html+='<input type="text" class="ez-dose-edit-input" data-idx="'+i+'" value="'+noteEsc+'" ';
    html+='style="width:100%;padding:5px 8px;border:1.5px solid rgba(129,140,248,0.2);border-radius:8px;font-size:12px;font-weight:700;font-family:Cairo,sans-serif;color:#1e1b4b;background:#fff;outline:none;direction:rtl;transition:border-color 0.2s" ';
    html+='onfocus="this.style.borderColor=\'#6366f1\'" onblur="this.style.borderColor=\'rgba(129,140,248,0.2)\'" />';
    html+='</div>';
    html+='</div>';
  }
  html+='</div>';
  html+='<div class="ez-doses-footer"><button onclick="window._ezApplyDoseEdits()" style="flex:1;height:42px;border:none;border-radius:12px;font-size:13px;font-weight:800;cursor:pointer;font-family:Cairo,sans-serif;color:#fff;background:linear-gradient(145deg,#10b981,#059669);box-shadow:0 4px 14px rgba(16,185,129,0.2)">✅ تطبيق التعديلات</button><button class="ez-btn-close-doses" onclick="window.ezCloseDoses()">✕ إغلاق</button></div>';
...
window._ezApplyDoseEdits=function(){
  var items=window._ezDoseItems;
  if(!items||!items.length){window.ezShowToast('لا توجد بيانات','info');return;}
  var inputs=document.querySelectorAll('#ez-doses-dialog .ez-dose-edit-input');
  var count=0;
  inputs.forEach(function(inp){
    var idx=parseInt(inp.getAttribute('data-idx'));
    var item=items[idx];
    if(!item||!item.noteCell) return;
    var newNote=inp.value.trim();
    if(!newNote) return;
    var noteInp=item.noteCell.querySelector('input,textarea');
    if(noteInp){
      if(noteInp.value===newNote) return;
      noteInp.value=newNote;
      _ezFire(noteInp);
    } else {
      if(item.noteCell.textContent.trim()===newNote) return;
      item.noteCell.textContent=newNote;
    }
    inp.style.borderColor='#10b981';
    inp.style.background='rgba(16,185,129,0.06)';
    count++;
  });
  if(count>0){
    window.ezShowToast('✅ تم تطبيق '+count+' تعديل على الجدول','success');
    ezBeep('success');
  } else {
    window.ezShowToast('لا يوجد تعديلات جديدة','info');
  }
};
