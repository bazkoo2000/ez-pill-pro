javascript:(function(){
  ...
  ui.innerHTML=`
    <div id="baz-header">
      <div class="hdr-left">
        ...
        <div><span class="hdr-title">البحث الشامل</span><br><span class="hdr-ver">v15.2 — Pure Speed & Data Match ⚡</span></div>
      </div>
      <div class="hdr-btns">
  ...
  const addCard=(res, index)=>{
    ...
    // ترتيب فليكس بوكس بصري مبني على الثواني
    let orderVal = (isNaN(t) || t === 0) ? 2000000000 : Math.floor(t / 1000);

    // تجهيز البيانات للرابط وحمايتها من الـ NA
    let cleanErx = String(erx).replace(/ERX/gi,'');
    if (cleanErx.toUpperCase() === 'NA' || cleanErx === '—') cleanErx = '';
    
    let cleanInv = String(invoice);
    if (cleanInv.toUpperCase() === 'NA' || cleanInv === '—') cleanInv = '';
    ...
    // تسجيل الرابط لزر الفتح المتعدد
      ...
      try {
        ...
        if (Array.isArray(orders) && orders.length > 0) {
          orders.forEach(item => {
            ...
            // استخراج فوري وخاطف للبيانات O(1)
            let erx = item.onlineNumber || item.Order_Number || item.increment_id || item.CUST_ID || item.ERX || item.erx || '';
            if (!erx || String(erx).toUpperCase() === 'NA') erx = q.ord || '—'; // تعويض ذكي باللي انت بحثت بيه

            let invoice = item.Invoice || item.invoice_id || item.invoice_number || '';
            if (!invoice || String(invoice).toUpperCase() === 'NA') invoice = q.inv || '—';
            ...
            // مفتاح صارم لمنع التكرار (يحل مشكلة הـ 5 نتائج)
            let key = invoice + '|' + erx;
            if (key === '—|—') key = item.entity_id || JSON.stringify(item).substring(0, 30);
            
            if (seen.has(key)) return;
            seen.add(key);
            count++;

            // استخراج الوقت بدقة للترتيب 
            let t = 0;
            if(dateRaw !== '—') {
            ...
            // تحديد الحالة الحقيقية من داخل الطلب
            let raw = String(item.status || item.Status || item.order_status || item.OrderStatus || '').toLowerCase().trim();
            let actualInfo = info;
            if(raw) {
            ...
            // طباعة فورية وسريعة جداً 
            addCard({ item, info: actualInfo, erx, invoice, guestName, guestMob, dateRaw, t }, 0);
          });
        }
      } catch(err) {
