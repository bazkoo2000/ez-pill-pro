...
window._ezAddDrugParseFile=function(file,cb){
  ...
  if(ext.endsWith('.csv')||ext.endsWith('.txt')){var fr=new FileReader();fr.onload=function(){cb(window._ezAddDrugParseCSV(String(fr.result||'')));};fr.readAsText(file);return;}
  if(window.XLSX){window._ezAddDrugReadXLSX(file,cb);return;}
  var s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js';s.onload=function(){window._ezAddDrugReadXLSX(file,cb);};s.onerror=function(){cb(null);};document.head.appendChild(s);
};
...
window._ezAddDrugReadXLSX=function(file,cb){
  ...
  fr.onload=function(e){
    try{
      var data=new Uint8Array(e.target.result);var wb=XLSX.read(data,{type:'array'});var ws=wb.Sheets[wb.SheetNames[0]];
      ...
      for(var h=0;h<head.length;h++){if(head[h]==='name'||head[h].indexOf('name')>-1||head[h].indexOf('اسم')>-1)iName=h;if(head[h]==='code'||head[h].indexOf('code')>-1||head[h].indexOf('كود')>-1)iCode=h;}
      if(iName<0)iName=0;if(iCode<0)iCode=1;
      var db=[];for(var i=1;i<rows.length;i++){var r=rows[i]||[];var nm=(r[iName]||'').toString().trim();var cd=(r[iCode]||'').toString().trim();if(nm||cd)db.push({name:nm,code:cd});}
      cb(db);
    }catch(err){cb(null);}
...
window._ezDoAddDrug=function(){
  ...
  var ql=q.toLowerCase();var item=null;
  ...
  tbody.appendChild(newRow);newRow.scrollIntoView({block:'center'});
