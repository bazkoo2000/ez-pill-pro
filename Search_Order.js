(function(){
    console.log("🚀 المحرك بدأ...");
    var d=document, id='ali_sys_v22';
    var old=d.getElementById(id); if(old)old.remove();

    var pNodes=Array.from(d.querySelectorAll('.pagination a, .pagination li')).map(el=>parseInt(el.innerText.trim())).filter(n=>!isNaN(n));
    ...
    var v=d.createElement('div'); v.id=id;
    ...
    var upC=(m)=>{ d.getElementById('stat_total').innerText=savedRows.length; d.getElementById('stat_rec').innerText=m||0; };
    ...
    var finish=()=>{
        ...
        var filter=()=>{
            ...
            savedRows.forEach(o=>{
                ...
                if(mI||mO||(v1==="0" && v2==="")){ if(table)table.appendChild(o.node); count++; }
            });
            upC(count); bM.innerText="فتح المطابق ("+count+")";
        };
