javascript:(function(){
        ...
        '<div id="ali_main_body">' +
          '<div style="background:#f8fafc;border:1px solid #f1f5f9;border-radius:16px;padding:16px;margin-bottom:16px">' +
            '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">' +
              '<span style="font-size:13px;font-weight:700;color:#475569">📄 تم رصد صفحات حتى:</span>' +
              '<div style="display:flex;align-items:center;gap:6px">' +
                '<span style="font-size:12px;color:#94a3b8;font-weight:600">صفحة</span>' +
                '<input type="number" id="p_lim" value="' + state.maxDetected + '" min="1" style="width:48px;padding:4px 6px;border:2px solid #e2e8f0;border-radius:8px;text-align:center;font-size:16px;font-weight:800;color:#3b82f6;background:white;outline:none;font-family:Segoe UI,Roboto,sans-serif">' +
              '</div>' +
            '</div>' +
            '<div style="font-size:10px;color:#94a3b8;text-align:center;margin-bottom:8px">* يتحدث تلقائياً أثناء التنقل *</div>' +
  ...
  function setStatus(text, type) {
    var el = document.getElementById('status-msg'); if (!el) return;
    var configs = { ready:{bg:'#f0fdf4',color:'#15803d',border:'#bbf7d0',icon:'✅'}, working:{bg:'#eff6ff',color:'#1d4ed8',border:'#bfdbfe',icon:'spinner'}, error:{bg:'#fef2f2',color:'#dc2626',border:'#fecaca',icon:'❌'}, done:{bg:'#f0fdf4',color:'#15803d',border:'#bbf7d0',icon:'🎉'}, sync:{bg:'#fefce8',color:'#a16207',border:'#fef08a',icon:'spinner'} };
    var c = configs[type] || configs.ready;
    var iconHTML = c.icon === 'spinner' ? '<div style="width:16px;height:16px;border:2px solid rgba(59,130,246,0.2);border-top-color:#3b82f6;border-radius:50%;animation:aliSpin 0.8s linear infinite;flex-shrink:0"></div>' : '<span>' + c.icon + '</span>';
    ...
    document.querySelectorAll('table tr').forEach(function(row) {
      ...
      if (cells.length > 1) {
        ...
        if (key.length > 3 && !state.visitedSet.has(key)) {
          ...
          if (label) { var m = label.getAttribute('onclick').match(/'(.*?)','(.*?)','(.*?)','(.*?)'/); if (m) args = [m[1], m[2], m[3], m[4]]; }
          if (!args) noArgsCount++;
  ...
  // ═══════════════════════════════════════════
  //  Scanner — منطق v4.4 بالظبط
  // ═══════════════════════════════════════════
  var totalNoArgs = 0;

  function scanPage(curr, isSync) {
    ...
    updateMaxPage();
    var limit = parseInt(document.getElementById('p_lim').value) || state.maxDetected;

    var fill = document.getElementById('p-fill');
    if (fill) fill.style.width = ((curr / limit) * 100) + '%';

    setStatus((isSync ? 'مزامنة' : 'تحليل') + ' الصفحة ' + curr + ' من ' + limit + ' ... تم رصد ' + state.savedRows.length + ' طلب', isSync ? 'sync' : 'working');

    var nextBtn = null;
    document.querySelectorAll('.pagination a, .pagination li, .pagination span').forEach(function(el) {
      if (el.innerText.trim() == String(curr + 1) || el.innerText.includes('»')) nextBtn = el;
    });

    if (curr < limit && nextBtn) {
      nextBtn.click();
      setTimeout(function() { scanPage(curr + 1, isSync); }, 11000);
    } else {
      finishScan(isSync);
    }
  }
  ...
  function finishScan(isSync) {
    ...
    if (totalNoArgs > 0) showToast(totalNoArgs + ' طلب بدون بيانات فتح (لن يتم فتحها)', 'warning');
    if (isSync) { setStatus('تمت المزامنة — ' + state.savedRows.length + ' طلب', 'done'); showToast('تمت المزامنة: ' + state.savedRows.length + ' طلب', 'success'); }
    else { setStatus('تم التجميع — ' + state.savedRows.length + ' طلب جاهز', 'done'); showToast('تم تجميع ' + state.savedRows.length + ' طلب بنجاح', 'success'); }

    var mainBody = document.getElementById('ali_main_body');
    mainBody.innerHTML =
      '<div style="margin-bottom:10px"><div style="position:relative"><span style="position:absolute;left:14px;top:50%;transform:translateY(-50%);font-size:17px;font-weight:900;color:#94a3b8;z-index:1;pointer-events:none;font-family:monospace">0</span><input type="text" id="ali_sI" placeholder="أدخل الأرقام بعد الـ 0 (كود الصيدلية = أول 4 أرقام)..." style="width:100%;padding:14px 16px 14px 34px;border:2px solid #e2e8f0;border-radius:12px;font-size:15px;font-family:Segoe UI,monospace;outline:none;background:#f8fafc;color:#1e293b;direction:ltr;text-align:left;transition:all 0.25s;letter-spacing:1px;font-weight:700;box-sizing:border-box"></div></div>' +
      '<div style="margin-bottom:10px"><div style="position:relative"><span style="position:absolute;right:14px;top:50%;transform:translateY(-50%);font-size:14px;z-index:1;pointer-events:none">🔗</span><input type="text" id="ali_sO" placeholder="بحث برقم الطلب (ERX)..." style="width:100%;padding:14px 42px 14px 16px;border:2px solid #e2e8f0;border-radius:12px;font-size:14px;font-family:Segoe UI,Roboto,sans-serif;outline:none;background:#f8fafc;color:#1e293b;direction:rtl;transition:all 0.25s;font-weight:600;box-sizing:border-box"></div></div>' +
      '<div id="ali_search_count" style="font-size:11px;color:#94a3b8;text-align:center;font-weight:600;padding:2px 0 12px">عرض ' + state.savedRows.length + ' من ' + state.savedRows.length + ' نتيجة</div>' +
      '<button id="ali_btn_open" style="width:100%;padding:14px 20px;border:none;border-radius:14px;cursor:pointer;font-weight:800;font-size:15px;font-family:Segoe UI,Roboto,sans-serif;display:flex;align-items:center;justify-content:center;gap:8px;background:linear-gradient(135deg,#059669,#10b981);color:white;box-shadow:0 4px 15px rgba(16,185,129,0.3);transition:all 0.3s;margin-bottom:8px">⚡ ابحث أولاً ثم افتح المطابق</button>' +
      '<button id="ali_btn_sync" style="width:100%;padding:12px 16px;border:none;border-radius:14px;cursor:pointer;font-weight:700;font-size:13px;font-family:Segoe UI,Roboto,sans-serif;display:flex;align-items:center;justify-content:center;gap:8px;background:#f8fafc;border:2px solid #e2e8f0;color:#475569;transition:all 0.3s">🔄 مزامنة (تحديث + حذف المُغلق + إضافة الجديد)</button>';

    var sI = document.getElementById('ali_sI'), sO = document.getElementById('ali_sO');
    var searchCount = document.getElementById('ali_search_count'), openBtn = document.getElementById('ali_btn_open');
    var currentMatches = [];
    ...
    function filterResults() {
      ...
      state.tbody.innerHTML = ''; var shown = 0; currentMatches = [];
      ...
      if (hasFilter && currentMatches.length > 0) { var openable = currentMatches.filter(function(r) { return r.args !== null; }).length; openBtn.innerHTML = '⚡ فتح المطابق (' + openable + ' طلب)'; openBtn.style.opacity = '1'; openBtn.style.cursor = 'pointer'; }
      else if (hasFilter && currentMatches.length === 0) { openBtn.innerHTML = '⚡ لا توجد نتائج مطابقة'; openBtn.style.opacity = '0.5'; openBtn.style.cursor = 'not-allowed'; }
    ...
    sI.addEventListener('input', debouncedFilter); sO.addEventListener('input', debouncedFilter);

    openBtn.addEventListener('click', async function() {
      var rawInvoice = sI.value.trim(), orderSearch = sO.value.trim().toLowerCase();
      if (rawInvoice === '' && orderSearch === '') { showToast('ابحث أولاً برقم الفاتورة أو رقم الطلب!', 'warning'); sI.focus(); sI.style.animation = 'aliBlink 0.5s 3'; setTimeout(function() { sI.style.animation = ''; }, 1500); return; }
      var openable = currentMatches.filter(function(r) { return r.args !== null; }), skipped = currentMatches.length - openable.length;
      if (openable.length === 0) { showToast(skipped > 0 ? skipped + ' طلب مطابق لكن بدون بيانات فتح!' : 'لا توجد طلبات مطابقة!', skipped > 0 ? 'error' : 'warning'); return; }
      var dialogInfo = [{ label: 'عدد الطلبات', value: openable.length + ' طلب', color: '#10b981' }, { label: 'الوقت المتوقع', value: '~' + Math.ceil(openable.length * 1.2) + ' ثانية', color: '#f59e0b' }];
      ...
      var result = await showDialog({ icon: '📂', iconColor: 'blue', title: 'فتح الطلبات المطابقة', desc: 'سيتم فتح ' + openable.length + ' طلب في نوافذ منفصلة', info: dialogInfo, buttons: [{ text: 'إلغاء', value: 'cancel' }, { text: '✅ تأكيد الفتح', value: 'confirm', style: 'background:linear-gradient(135deg,#059669,#10b981);color:white;box-shadow:0 4px 12px rgba(16,185,129,0.3)' }] });
      if (result !== 'confirm') return;
      openBtn.disabled = true; openBtn.style.opacity = '0.6'; openBtn.style.cursor = 'not-allowed';
      var opened = 0, failed = 0, base = window.location.origin + "/ez_pill_web/getEZPill_Details";
      for (var idx = 0; idx < openable.length; idx++) {
        var item = openable[idx];
        var url = base + "?onlineNumber=" + item.args[0].replace("ERX", "") + "&Invoice=" + item.args[1] + "&typee=" + item.args[2] + "&head_id=" + item.args[3];
        try { var w = window.open(url, "_blank"); if (w) { opened++; state.openedCount++; } else { failed++; } } catch (e) { failed++; }
        openBtn.innerHTML = '🚀 جاري الفتح (' + (idx + 1) + '/' + openable.length + ')';
        setStatus('فتح ' + (idx + 1) + ' من ' + openable.length + ': ' + (item.onl || item.id), 'working'); updateStats();
        if (idx < openable.length - 1) await new Promise(function(resolve) { setTimeout(resolve, 1200); });
      }
      if (failed > 0) { await showDialog({ icon: '⚠️', iconColor: 'red', title: 'تنبيه', desc: 'تعذر فتح ' + failed + ' نافذة — تأكد من السماح بالنوافذ المنبثقة', info: [{ label: 'تم فتحها', value: opened.toString(), color: '#10b981' }, { label: 'فشلت', value: failed.toString(), color: '#ef4444' }], buttons: [{ text: '👍 حسناً', value: 'ok', style: 'background:linear-gradient(135deg,#1e40af,#3b82f6);color:white' }] }); }
      else { await showDialog({ icon: '🎉', iconColor: 'green', title: 'تم بنجاح!', desc: 'تم فتح جميع الطلبات المطابقة', info: [{ label: 'تم فتحها', value: opened + ' طلب', color: '#10b981' }, { label: 'إجمالي المفتوح', value: state.openedCount + ' طلب', color: '#3b82f6' }], buttons: [{ text: '👍 إغلاق', value: 'ok', style: 'background:linear-gradient(135deg,#1e40af,#3b82f6);color:white' }] }); }
    ...
    document.getElementById('ali_btn_sync').addEventListener('click', async function() {
      ...
      var syncBtn = this, oldCount = state.savedRows.length;
      var result = await showDialog({ icon: '🔄', iconColor: 'blue', title: 'المزامنة الذكية', desc: 'سيتم إعادة فحص الصفحات لتحديث القائمة', info: [{ label: 'الطلبات الحالية', value: oldCount.toString(), color: '#8b5cf6' }, { label: 'العملية', value: 'حذف المُغلق + إضافة الجديد', color: '#3b82f6' }, { label: 'الصفحات', value: state.maxDetected + ' صفحة', color: '#f59e0b' }], buttons: [{ text: 'إلغاء', value: 'cancel' }, { text: '🔄 بدء المزامنة', value: 'confirm', style: 'background:linear-gradient(135deg,#1e40af,#3b82f6);color:white;box-shadow:0 4px 12px rgba(59,130,246,0.3)' }] });
      if (result !== 'confirm') return;
      state.isSyncing = true; syncBtn.disabled = true;
      ...
      state.visitedSet.clear(); state.savedRows = []; state.maxDetected = 1; totalNoArgs = 0;
      scanPage(1, true);
    });
  }
