...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
var CHANGELOG={
  '146.0':{    title:'🔧 تحسينات شاملة + دقة محلية',    features:[      {icon:'🚫',text:'عند الحاجة/اللزوم/PRN: يُزال تلقائياً مثل الثلاجة'},      {icon:'🕐',text:'أوقات سعودية جديدة: الضحى/الفجر/بعد صلاة'},      {icon:'⚡',text:'تسريع فتح الدايلوج + تأخير فحص الجدول'},      {icon:'🔤',text:'توحيد تطبيع النصوص العربية في دالة واحدة'},      {icon:'👤',text:'كلمات توقف إضافية لاستخلاص الاسم'},      {icon:'🔧',text:'استبدال أوقات مكتوبة بـ NORMAL_TIMES'}    ]  },
  '145.0':{
...
window.ezCloudSettings=function(){
  ...
  if(!isAdmin){
    ...
    var cwTime=document.createElement('input');cwTime.type='time';cwTime.value='09:00';cwTime.style.cssText='width:140px;padding:8px;border:1.5px solid rgba(129,140,248,0.2);border-radius:8px;font-size:13px;font-family:Cairo,sans-serif;outline:none';
    ...
    var cstTime=document.createElement('input');cstTime.type='time';cstTime.value='21:00';cstTime.style.cssText='width:140px;padding:8px;border:1.5px solid rgba(129,140,248,0.2);border-radius:8px;font-size:13px;outline:none';
...
function ramadanMapNote(note){
  ...
  if(/مع\s*(ال)?(فطار|فطور|افطار|فكار)/i.test(note)) return {meal:'afterIftar',label_ar:'بعد الفطار',label_en:'After Iftar',time:RAMADAN_TIMES.afterIftar};
  ...
  if(/قبل.*فطار|قبل.*فطور|قبل.*افطار|قبل.*فكار|before.*iftar|before.*breakfast/i.test(note)) return {meal:'beforeIftar',label_ar:'قبل الفطار',label_en:'Before Iftar',time:RAMADAN_TIMES.beforeIftar};
  /* بعد الفطار / after iftar */
  if(/بعد.*فطار|بعد.*فطور|بعد.*افطار|بعد.*فكار|after.*iftar|after.*breakfast/i.test(note)) return {meal:'afterIftar',label_ar:'بعد الفطار',label_en:'After Iftar',time:RAMADAN_TIMES.afterIftar};
  ...
  if(/قبل.*فطار|قبل.*فطر|قبل.*فطور|قبل.*افطار|قبل.*فكار|before.*iftar|before.*bre/i.test(note)) return {meal:'beforeIftar',label_ar:'قبل الفطار',label_en:'Before Iftar',time:RAMADAN_TIMES.beforeIftar};
  /* بعد الفطار / after iftar / breakfast */
  if(/بعد.*فطار|بعد.*فطر|بعد.*فطور|بعد.*افطار|بعد.*فكار|after.*iftar|after.*bre/i.test(note)) return {meal:'afterIftar',label_ar:'بعد الفطار',label_en:'After Iftar',time:RAMADAN_TIMES.afterIftar};
...
/* ══ v146: Unified Arabic Normalization ══ */
...
function smartDoseRecognizer(note){
  ...
  res.hasB=/\b(bre|breakfast|fatur|ftor|iftar)\b|فطر|فطار|فطور|افطار|الافطار|الفطور|الفطار|فكار|الفكار|مع\s*(ال)?(فطار|فطور|افطار|فكار)/i.test(s);
...
function getTwoPillsPerDoseInfo(n){
  ...
  var twoP=['2 حبه','2 حبة','حبتين','حبيتين','حبتان','2 حبوب','2 قرص','قرصين','قرصان','2 كبسوله','كبسولتين','كبسولتان','2 pill','2 pills','two pill','two pills','2 tablet','2 tablets','two tablet','two tablets','2 tab','2 tabs','two tab','two tabs','2 cap','2 caps','two cap','two caps'];
...
function getTimeFromWords(w){
  ...
  var _mA=/بعد\s*(الاكل|الأكل|الآكل|الوجبه?|الطعام)|after\s*(meal|food)\b|\bpc\b/i.test(s);
  var _mB=/قبل\s*(الاكل|الأكل|الآكل|الوجبه?|الطعام)|before\s*(meal|food)\b|\bac\b/i.test(s);
  if(_mA||_mB){
  ...
  if(/(?:[بي]عد|مع)\s*(الاكل|الأكل|الآكل|الوجبه?|الطعام)|after\s*(meal|food)\b|\bpc\b|\bp\.c\b/i.test(s))return{time:NT.afterBreakfast||'09:00'};
  /* قبل الاكل المبهمة → قبل الفطار */
  if(/قبل\s*(الاكل|الأكل|الآكل|الوجبه?|الطعام)|before\s*(meal|food)\b|\bac\b/i.test(s))return{time:NT.beforeMeal||'08:00'};
  var beforeMealTwice=/قبل\s*(الاكل|الأكل)\s*مرتين|مرتين\s*قبل\s*(الاكل|الأكل)|before\s*(meal|food)\s*twice|twice\s*before\s*(meal|food)/;
  if(beforeMealTwice.test(s))return{time:NT.beforeMeal};
...
function getMealTimesFromNote(note){
  var s=(note||'').toLowerCase().replace(/[أإآ]/g,'ا').replace(/ة/g,'ه').replace(/ى/g,'ي').trim();
  /* v146: Per-keyword before/after instead of global isBefore */
  var bfBefore=/قبل\s*(ال)?(فطار|فطور|افطار|فكار)|before\s*(break|iftar)/i.test(s);
  var bfAfter=/بعد\s*(ال)?(فطار|فطور|افطار|فكار)|after\s*(break|iftar)|مع\s*(ال)?(فطار|فكار)/i.test(s);
  var lnBefore=/قبل\s*(ال)?(غدا|غداء|غذا|غذاء)|before\s*lunch/i.test(s);
  var lnAfter=/بعد\s*(ال)?(غدا|غداء|غذا|غذاء)|after\s*lunch|مع\s*(ال)?(غدا|غداء)/i.test(s);
  var dnBefore=/قبل\s*(ال)?(عشا|عشاء)|before\s*(din|sup)/i.test(s);
  var dnAfter=/بعد\s*(ال)?(عشا|عشاء)|after\s*(din|sup)|مع\s*(ال)?(عشا|عشاء)/i.test(s);
  var globalBefore=!bfBefore&&!bfAfter&&!lnBefore&&!lnAfter&&!dnBefore&&!dnAfter&&/قبل/i.test(s)&&!/بعد/i.test(s);

  var hasB=/فطر|فطار|فطور|افطار|الفطار|فكار|الفكار|breakfast|fatur|ftor/i.test(s);
...
function processTable(m,t,autoDuration,enableWarnings,showPostDialog,ramadanMode){
  ...
  function createDuplicateRows(t_val,r,ni,bs,niIdx,si,ei,di,ti,sdi,edi,m_val,tc,ci,qi){
    ...
    if(ni.type==='two'){
      ...
      var n1='',t1='',n2='',t2='';
      /* v146: NEVER rename notes. Keep original text, only set different times */
      var _origNote=get(r.querySelectorAll('td')[niIdx]);
      var _mealTimes=getMealTimesFromNote(_origNote);
      if(_mealTimes.length>=2){
        t1=('0'+Math.floor(_mealTimes[0])).slice(-2)+':'+('0'+Math.round((_mealTimes[0]%1)*60)).slice(-2);
        t2=('0'+Math.floor(_mealTimes[1])).slice(-2)+':'+('0'+Math.round((_mealTimes[1]%1)*60)).slice(-2);
      } else if(ni.doseInfo.hasM&&ni.doseInfo.hasN){t1=NORMAL_TIMES.morning;t2=NORMAL_TIMES.noon;}
      else if(ni.doseInfo.hasN&&ni.doseInfo.hasE){t1=NORMAL_TIMES.noon;t2=NORMAL_TIMES.evening;}
      else if(ni.doseInfo.hasM&&ni.doseInfo.hasA){t1=NORMAL_TIMES.morning;t2=NORMAL_TIMES.afternoon;}
      else if(ni.doseInfo.hasA&&ni.doseInfo.hasE){t1=NORMAL_TIMES.afternoon;t2=NORMAL_TIMES.evening;}
      else if(ni.doseInfo.hasB&&ni.doseInfo.hasL){t1=ni.isBefore?NORMAL_TIMES.beforeBreakfast:NORMAL_TIMES.afterBreakfast;t2=ni.isBefore?NORMAL_TIMES.beforeLunch:NORMAL_TIMES.afterLunch;}
      ...
      var _origNote3=get(r.querySelectorAll('td')[niIdx]);
      var _mealTimes3=getMealTimesFromNote(_origNote3);
      if(_mealTimes3.length>=3){
        t1=('0'+Math.floor(_mealTimes3[0])).slice(-2)+':'+('0'+Math.round((_mealTimes3[0]%1)*60)).slice(-2);
        t2=('0'+Math.floor(_mealTimes3[1])).slice(-2)+':'+('0'+Math.round((_mealTimes3[1]%1)*60)).slice(-2);
        t3=('0'+Math.floor(_mealTimes3[2])).slice(-2)+':'+('0'+Math.round((_mealTimes3[2]%1)*60)).slice(-2);
    ...
    if(ramadanMode){
      ...
      /* ── CASE D: Once daily ── */
        ...
        if(!noteMapR){
          ...
          var _exFutur2=/فطار|فطور|افطار|فكار|iftar|breakfast/i.test(fn_str);
        ...
        if(!noteMapR && (doseRec.hasB||doseRec.hasL||doseRec.hasD||doseRec.hasM||doseRec.hasN||doseRec.hasA||doseRec.hasE||doseRec.hasBed||doseRec.hasEmpty)){
          ...
          var _hasExplicitFutur=/فطار|فطور|افطار|فكار|iftar|breakfast/i.test(fn_str);
  ...
  if(_ezGetGeminiKey()){
    for(var _gScan=0;_gScan<allRowsData.length;_gScan++){
      ...
      /* v146: Fixed size items still need Gemini for TIME resolution */
      if(_gRd.note&&_gRd.note.trim().length>=3&&!_gRd.isWeekly){
  ...
  if(enableWarnings){
    for(var i=0;i<allRowsData.length;i++){
      ...
      /* Check for unrecognized time patterns - v146: fixed items still need time warnings */
      if(rd.note&&rd.note.trim().length>=3&&!rd.isWeekly){
  ...
  /* v146 DEBUG: toast for unrecognized count */
  var _unrecCount=0;
  for(var _dbg=0;_dbg<allRowsData.length;_dbg++){
    if(allRowsData[_dbg]._needsGemini){
      console.log('🤖 Row '+_dbg+' needs Gemini: "'+allRowsData[_dbg].note+'"');
      _unrecCount++;
    }
  }
  var _hasKey=!!_ezGetGeminiKey();
  console.log('🤖 Gemini: '+_geminiNotes.length+' unrecognized notes, key='+_hasKey);
  if(_geminiNotes.length>0){console.log('🤖 Notes to send:',_geminiNotes);}
  console.log('🤖 warningQueue length: '+warningQueue.length+', enableWarnings: '+enableWarnings);
  if(_geminiNotes.length>0&&_ezGetGeminiKey()){
    ...
    _ezGeminiBatch(_geminiNotes).then(function(results){
      ...
      window.ezShowToast('🤖 جيميناي رد بنجاح!','success');
  ...
  function continueProcessing(){
    ...
    for(var i=0;i<allRowsData.length;i++){
      ...
      /* v146: Fixed codes with notes: use note for time/every but keep fixed size */
      if(rd.hasFixedSize&&!rd.warningOverride&&_fixedHasNote){
        var _fixSizeN=rd.fixedSizeBreak||fixedSizeCodes[rd.itemCode];
        ...
        '+(function(){
          ...
          if(cc.customRamadanRules&&cc.customRamadanRules.length>0){
            ...
            for(var i=0;i<cc.customRamadanRules.length;i++){
              ...
              customHtml+='<div style="display:flex;align-items:center;gap:6px;padding:6px 10px;background:rgba(251,191,36,0.04);border-radius:10px;border:1px solid rgba(251,191,36,0.08);margin-bottom:6px"><span style="font-size:12px;width:18px;text-align:center">⭐</span><span style="flex:1;font-size:11px;font-weight:700;color:#64748b;direction:rtl">'+cr.label+'</span><input type="time" id="cfg-rm-custom-'+i+'" value="'+cr.time+'" data-pattern="'+cr.pattern+'" style="width:140px;padding:4px 8px;border:1.5px solid rgba(251,191,36,0.15);border-radius:8px;font-size:13px;font-weight:800;font-family:Cairo,sans-serif;color:#1e1b4b;outline:none;text-align:center" /><button class="ez-cfg-del-rm-custom" data-idx="'+i+'" style="width:24px;height:24px;border:none;border-radius:6px;background:rgba(239,68,68,0.06);color:#ef4444;cursor:pointer;font-size:10px;flex-shrink:0">✕</button></div>';
            }
            customHtml+='</div>';
          }
          return customHtml;
        ...
        '+(function(){
          ...
          if(cc.customTimeRules&&cc.customTimeRules.length>0){
            ...
            for(var i=0;i<cc.customTimeRules.length;i++){
              ...
              customHtml+='<div style="display:flex;align-items:center;gap:6px;padding:6px 10px;background:rgba(139,92,246,0.04);border-radius:10px;border:1px solid rgba(139,92,246,0.08);margin-bottom:6px"><span style="font-size:12px;width:18px;text-align:center">⭐</span><span style="flex:1;font-size:11px;font-weight:700;color:#64748b;direction:rtl">'+cr.label+'</span><input type="time" id="cfg-nt-custom-'+i+'" value="'+cr.time+'" data-pattern="'+cr.pattern+'" style="width:140px;padding:4px 8px;border:1.5px solid rgba(139,92,246,0.15);border-radius:8px;font-size:13px;font-weight:800;font-family:Cairo,sans-serif;color:#1e1b4b;outline:none;text-align:center" /><button class="ez-cfg-del-nt-custom" data-idx="'+i+'" style="width:24px;height:24px;border:none;border-radius:6px;background:rgba(239,68,68,0.06);color:#ef4444;cursor:pointer;font-size:10px;flex-shrink:0">✕</button></div>';
            }
            customHtml+='</div>';
          }
          return customHtml;
...
/* v146: hasDuplicateNotes deferred to after dialog renders for speed */
var hasDuplicateNotes=false;
...
/* v146: ALL scanning deferred → dialog opens instantly */
setTimeout(function(){
  try{
  ...
  try{
    ...
    <div class="eid-card">\
      ...
      <div class="eid-body">\
        ...
        <div class="eid-from">EZ_Pill Farmadosis · علي الباز</div>\
    ...
    (function(){
      ...
      if(_afterSlash){
        ...
        for(var _sw=0;_sw<_slashWords.length;_sw++){
          ...
          var _swNorm=_slashWords[_sw].replace(/[أإآ]/g,'ا').replace(/ة/g,'ه').trim();
          ...
          if(_slashCleaned.length>0&&/^و/.test(_slashWords[_sw])&&_slashWords[_sw].length>1){
            ...
            if(/^(التوصيل|توصيل|شكرا|الصيدلي[ةه]|صيدلي[ةه]|رقم|الرقم|تواصل|التواصل)$/i.test(_afterW.replace(/[أإآ]/g,'ا').replace(/ة/g,'ه'))) break;
          }
